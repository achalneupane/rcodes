...
# /media/UQCCG/Programming/VersionControl_GitRepository/UQCCG_Pipeline_Rscripts/Analyse_project_new_Aug_2014.r
# ###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
# UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project dataUE
...
# analysis.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2013-10-27_AML_with_AOGCControl_NoFailedLane/Analysis"
#
...
UQCCG.data <- args[2]
project <- args[3]
the.sample.sheet<- args[4]


force.functional.update<-FALSE # will re-do functional (pholyPhen) data normally set to FALSE
force.GERP.update<-FALSE  # will re-do GERP  get scores normally set to FALSE
update.annotations.with.vep<-TRUE ## TRUE normally undate gene names and geneanno.table with filter.table.poly... wanted muts traken care of below
update.gene.names<-TRUE # TRUE normally
...
analysis.dir<-paste(UQCCG.data,project,"Analysis",sep="/")
 ...
 affection.status<-"sample_sheet"
#### the.sample.sheet<-"SDDS_AffStatus.csv" ## set in  args[4] above
 restrict.family<-c()




code.dir<-"/dmf/uqdi/Core_Services/UQCCG/Programming/VersionControl_GitRepository/UQCCG_Pipeline_Rscripts/"
annovar.excutable.path<-"/dmf/uqdi/Core_Services/UQCCG/Software/annovar"
Biomart.desc.file<-"/mnt/UQCCG/Software/annovar/humandb/Biomart.gene.desc.RData"
VEP.excutable.path<-"/dmf/uqdi/Core_Services/UQCCG/Software/VEP" # variant_effect_predictor.pl




filter.cols.novel<-c("PopFreqMax","NHBLI_6500_ANNOVAR_ALL","NHBLI_6500_ALL","NHLBI_5400_ALL","NHLBI_5400_EUR","NHLBI_5400_AFR","1000genome","1000genome_asian","1000genome_mine","snp141","snp141_clinical","snp137","CG69","EUR_ASN_AFR_INDEL","AOGC-NGS_ALL","AOGC-NGS_ALL_OLD","Chinese") ##
filter.cols.maf<-c("NHBLI_6500_ANNOVAR_ALL","NHBLI_6500_ALL","NHBLI_6500_EA","NHBLI_6500_AA","NHLBI_5400_ALL","1000genome","snp141","snp137")
...
alt.allele.other.use<-TRUE
make.nonDefined.samples.as.OTHER<-TRUE #### this is the default analysis all samples not in sample sheet are used as OTHER
include.homozygotes<-TRUE ## used for cancer
###############################################
...
library(doMC)
if(!exists("num.cores")){num.cores<-7}
...
if(!exists("annotate.dir")){annotate.dir<-"/dmf/uqdi/Core_Services/UQCCG/Sequencing/Data/Sequence_Genotypes"}
...
if(!exists("make.nonDefined.samples.as.OTHER")){make.nonDefined.samples.as.OTHER<-TRUE}  ## other samples not listed in the sample sheet are used at other
...
#######################
if(!exists("alt.allele.other.use.threshold") & alt.allele.other.use ){alt.allele.other.use.threshold<--9}  # if alt.allele.other.use.threshold not set , alt allelse set using binomial model
...
#anno.DB.location<-"/media/Bioinform-D/Research/annovar/humandb/hg19" ## location of the annotation database
...
maf.threshold.filter.to.use<-c(0.001,0.005,0.01,0.025,0.05) #c(0.001,0.01,0.025,0.05) MAF threshold for boolean filtering operations NOVEL always done
...
###############
sd.thresh<-6 ## used for alt.alleles.threshold
...
maf.lt.all<-data.frame(key=key.indels,stringsAsFactors=FALSE)
  ...
  for(imaff in 1:length(a.filter.cols.maf)){
    if(maf.threshold.filter.all[imaf]==0){ # different case for novel NOT found (rather than less than)
       maf.lt<-maf.lt & !filter.table[,a.filter.cols.maf[imaff]] ### this not strictly correct should use filter.cols.novel
     }else{
...
gene.lists<-c("Neurodegen","MND_omim","ALS_ji","omim","ingenuity.bone.genes","Dequeant.cycling","mouse.defect","sewell.cycling","lung.genes","skeletome","pot.genes","HypoMag","ProtonPump","AML_OTHER_Mutations","AML_MITO_Mutations","AML_LEY_RELAPSE_Mutations","AML_FRANC_Mutations","AML_ENU_Mutations","AML_DINDEL_Relapse","AML_SNP_Mutations","AML_DINDEL_Mutations","AML_CLINICAL_Mutations","AML_ASH_Mutations","AML_ALL_Mutations")
...
##### ADD CODE HERE to redo gerp.unknown ####
...
### save.image("project_chr16_test.RData")
...
###################################################  RESTART RESTART RESTART  ################################################################
###################################################  RESTART RESTART RESTART  ################################################################
if(length(all.sample.labels)>0){ # all.sample.labels==0 means X1it is a plink.assoc or bim file annotation run
...
if(include.control.in.other){
  the.samples.other<-unique(c(the.samples.other,sample.sheet.full[sample.sheet.full[,"SampleProject"]=="CONTROL","ParticipantCode"],sample.sheet.full[sample.sheet.full[,"SampleProject"]=="control","ParticipantCode"]))
}
...
############################### IN GROUP filter options frequency dependend #################################
n<-max(as.integer(summary.geno.other[,"TOTAL.Alleles.OTHER"]))
p<-maf.threshold.filter.all[imaf]
# p<-0.001 ########### set MAF threshols HERE
if(p==0){alt.counts.thresh<-0
       }else{
alt.counts.thresh<-1
while( (alt.counts.thresh- n*p) / sqrt(n*p*(1-p)) <= sd.thresh){alt.counts.thresh<-alt.counts.thresh+1}
}
alt.counts.thresh
ok.in.group<-as.numeric(summary.geno.other[,"ALT.Alleles.OTHER"]) <= alt.counts.thresh # in case is fully recessive

################## is want a fixed fraction then alt.allele.other.use.threshold must be defined
if(alt.allele.other.use.threshold!=-9){
  ...
  ok.in.group<-  alt.alleles.other.test
}
sum(ok.in.group)
#################################################
...
print("gene.desc.table")
print(length(gene.desc.table.labels.wanted))

print("unwanted.columns")
print(length(unwanted.columns))
#print(unwanted.columns)
...
if(genome.build=="hg19" |  genome.build=="hg18"){
  if(!Skip.Analysis){
    print("Not Skip Analysis")
      print(nrow(indels[,c(core.ann)]))
      print(length(geneanno.table))
      print(length(Gene.Names))
      print(length(gene.desc.table[,gene.desc.table.labels.wanted]))
      print(length(gene.table))
      print("filter.table.pholy")
      print(nrow(filter.table.pholy[,filter.table.annotation]))
      # Error Appears to be happening here
      # Above: 14361
      # Below: 17360
      print(nrow(indels[,filter.ID.cols]))
      print(length(wanted.muts))
      print(length(wanted.muts.coding))
      print(length(good.qual))
      print(length(wanted.muts.fil))
      print(length(ok.in.group))
      print(length(maf.lt.all))
      print(length(Denovo.DOM))
      print(length(Compound.Hetero))
      print(length(gerp.scores))
      print(length(summary.geno))
      print(length(summary.geno.group))
      print(length(summary.geno.other))
      print(length(summary.depths))
      print(nrow(indels[,indels.order.GT.AN]))
      print(nrow(indels[,indels.order.SNP.QUAL]))
      print("filter.table.pholy")
      print(nrow(filter.table.pholy[,c("PolyPhen.desc","PolyPhen.scores","SIFT.desc","SIFT.scores")]))
      # Error Appears to be happening here
      # Above: 14361
      # Below: 17360
      print(length(dom.profile))
      print(length(dom.aff.ALT.counts))
      print(length(dom.Unaff.REF.counts))
      print(length(dom.aff.ALT.counts.C))
      print(length(dom.Unaff.REF.counts.C))
      print(length(gene.hit.count.fil.all))
      print(length(gene.hit.count.fil.aff.all))
      print(length(gene.hit.count.fil.Unaff.all))
      print(length(phenotype.gene.hit.maf.qual.all))
      print(length(phenotype.gene.hit.maf.qual.all.Unaff))
      print(length(phenotype.gene.hit.count.maf.qual.all))
      print(length(phenotype.gene.hit.count.maf.qual.all.Unaff))
      print(length(phenotype.gene.hit.maf.qual.cov.all))
      print(length(phenotype.gene.hit.maf.qual.cov.all.Unaff))
      print(length(gene.hit.count.sample.all))
      print(length(summary.depths.group))
      print(length(gene.desc.table[,gene.desc.table.labels.other]))
      print(length(regionanno.table))
      print(length(indels[,indels.order.2nd]))
      print(length(filter.table))
      print(length(indels[,indels.order.other]))
...
print("other.sample.info")
print(length(other.sample.info))
print(other.sample.info)
