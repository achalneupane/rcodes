...
maf.threshold.filter.to.use<-c(0.001,0.01,0.025,0.05)
...
PD[pheno$SampleProject %in% c("Control")]<-TRUE
...
ok.missing & hw.controls.ok.filt & !no.genotypes &  rare.in.Control.filt & !in.any.normal.filt & !in.any.normal  # &  snp.only & !on.x.y
}
...
#save.image(file=paste("LEO_Pharma_WANTED_muts_Feb05",".RData",sep=""))
...
out<-cbind(meta.results.burden.ex,annotations[figure,],summary.geno.extra[figure,colnames(summary.geno.extra)[grep("^GENO",colnames(summary.geno.extra))]],help[figure,],muts.in.cases,quality.cases,muts.in.controls,quality.controls)
