...
column.labels<-read.delim(sample.files[i],header=F,nrows=1,skip=(skip.lines-1),sep="\t",fill=TRUE,stringsAsFactors=FALSE,quote="\"")
num.vars<-dim(column.labels)[2]

vcf.head<-read.delim(sample.files[i],header=F,nrows=(skip.lines-1),skip=0,sep="\t",fill=TRUE,stringsAsFactors=FALSE,quote="\"")
...
indels<-try(scan(sample.files[i],what=character(num.vars),skip=skip.lines,sep="\t",fill=TRUE,quote="\""))
...
indels<- process.GATK.indels(indels,sample.files[i],vcf.type,format.types,info.types,info.class,num.cores)
