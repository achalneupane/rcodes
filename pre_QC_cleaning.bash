...
awk '{if($5 =="I" || $5 == "D" || $6 == "I" || $6 == "D") print $0}' 04_${name}_SNPs_updated_alleles_no_unkown_pos_indel_update2.bim > test.txt
...
plink1.9 --bfile 04_${name}_SNPs_updated_alleles_no_unkown_pos_indel_update2 --allow-no-sex --keep-allele-order --list-duplicate-vars

cut -f 4 plink.dupvar > dups_from_plink.txt
awk 'gsub(/ /,"\n")' dups_from_plink.txt > dups_from_plink_1col.txt

echo "Number of duplicate variables identified by Plink --list-duplicate-vars: " >> report_1_pre_QC.txt
wc -l dups_from_plink_1col.txt >> report_1_pre_QC.txt

# extract thes duplicates, to calculate missingness (call rate)
plink1.9 --bfile 04_${name}_SNPs_updated_alleles_no_unkown_pos_indel_update2 --allow-no-sex --keep-allele-order --extract dups_from_plink_1col.txt --make-bed --out 04_duplicates_SNPs
...
#remove comment from vcf header
awk '/^#CHROM/{sub(/^#/,"")} 1' 04_duplicates_SNPs.vcf > 04_duplicates_SNPs_edited.vcf

## identify duplicates in R and use the list to exclude in plink ( dups_to_be_excluded.txt)
...
############### if my manifest file did not have all variants with one allele zero could not be resolved, those will be removed for the rest of the analysis
