...
## grep("10172",all.sample.labels)
## all.sample.labels[88]
## extra<-sample.sheet.full[,"ParticipantCode"][!(sample.sheet.full[,"ParticipantCode"] %in% all.sample.labels  )]
## extra.true<-gsub("MND_","MND-",extra)
## extra.true<-gsub("CH_MND","CH-MND",extra)

## posns<-match(extra,sample.sheet.full[,"ParticipantCode"])
## sample.sheet.full[posns,"ParticipantCode"]<-extra.true
##   write.table(sample.sheet.full,file="/mnt/UQCCG/Sequencing/Data/Sequence_Genotypes/2014-03-19_MND_MODY_LKAF_Nimbelgen/BAM/Ch_MND_SampleSheet.csv",col.names=TRUE,row.names=FALSE,sep=",",quote=FALSE)
...
new.controls<-the.samples.other
the.samples.other<-the.Unaffected
the.Unaffected<-new.controls

names(the.Unaffected)<-rep("UnAff",times=length(the.Unaffected))

the.samples<-c(the.affected,the.Unaffected,the.unknown)
sample.labels<-the.samples
...
file.out.name<- paste(file.out.name,"CONTROL_SWAP",sep=".")
