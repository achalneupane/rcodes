...
## use.samples<-the.samples[pheno.ori[,"AML"]]

genotype.GQ.summary<-function(indels,use.samples="all",suffix=""){
  if(is.null(dim(indels))){indels<-matrix(data=indels,nrow=1,ncol=length(x))}
  if(length(use.samples)==1){
  if(use.samples=="all"){ use.samples<- colnames(indels)[grepl(".GT$",colnames(indels))]}
             }
    use.samples.GQ<-gsub(".GT$",".GQ",use.samples)
    summary.geno<-genotype.summary(indels[,use.samples])
    colnames(summary.geno)<-c("MAF","ALT.Alleles","REF.Alleles","TOTAL.Alleles","MISSING.Alleles","ALT_HOMO","ALT_HETRO","GENO")
    has.a.call<-as.integer(summary.geno[,"ALT.Alleles"])>0 & !is.na(summary.geno[,"ALT.Alleles"])
    sum(has.a.call)

 # x<-indels[has.a.call,c(use.samples,use.samples.GQ)][1,]
    GQ<-apply(indels[has.a.call,c(use.samples,use.samples.GQ)],1,function(x) {
        the.GQ<-names(x)[grepl(".GQ",names(x))]
        the.GT<-names(x)[grepl(".GT",names(x))]
        genos<-x[the.GT]
        hits<-names(genos)[genos=="0/1" | genos=="1/1"]
        hits<-gsub(".GT$",".GQ",hits)
        x<-as.integer(x[hits])
        the.max<-max(x,na.rm=TRUE)
        the.min<-min(x,na.rm=TRUE)
        the.mean<-mean(x,na.rm=TRUE)
        the.median<-median(x,na.rm=TRUE)
        the.mean.trim<-mean(x,na.rm=TRUE,trim=0.1)
        cbind(the.max, the.min,  the.mean,the.median,the.mean.trim,length(x))
    } )

  GQ<-t(GQ)
  colnames(GQ)<-c("the.max","the.min","the.mean","the.median","the.mean.trim","number")
  if(suffix!=""){ colnames(GQ)<-paste( colnames(GQ),suffix,sep=".")}
  posns<-match(rownames(indels),rownames(GQ))
  GQ<-GQ[posns,]
  rownames(GQ)<-rownames(indels)
  return(GQ)
}
...
##   summary.depths.group.threshold<-apply(allele.depths.group,1,allele.summary.with.threshold.apply,2)
...
## alt.reads.Non.reference.calls<-function(indels,the.samples,threshold=1,prefix="",suffix=""){
##   have.AD<-paste(the.samples,"AD",sep=".") %in% colnames(indels)
##   if(sum(have.AD)!=0){
##   allele.depths.group<-indels[,paste(the.samples,"AD",sep=".")]
##   genotypes<-indels[,paste(the.samples,"GT",sep=".")]
...
## indels<-a.indel.stats
## the.samples<-novoalign
## AD.extension="FAD"
## threshold=1



alt.reads.reference.calls<-function(indels,the.samples,AD.extension="AD",threshold=1,prefix="",suffix=""){
  ### threshold has not been used
  have.AD<-paste(the.samples,AD.extension,sep=".") %in% colnames(indels)
  if(sum(have.AD)!=0){

   ##  sum(!(paste(the.samples,AD.extension,sep=".") %in% colnames(indels)))
   ##  sum((paste(the.samples,AD.extension,sep=".") %in% colnames(indels)))
   ## the.samples[!(paste(the.samples,AD.extension,sep=".") %in% colnames(indels))]
...
alt.reads.Non.reference.calls<-function(indels,the.samples,AD.extension="AD",threshold=1,prefix="",suffix=""){
  have.AD<-paste(the.samples,AD.extension,sep=".") %in% colnames(indels)
  if(sum(have.AD)!=0){
  allele.depths.group<-indels[,paste(the.samples,AD.extension,sep=".")]
  genotypes<-indels[,paste(the.samples,"GT",sep=".")]
...
## SnpStats.R<-function(indels,the.samples,mode="stats",AD.extension="AD",threshold=1,prefix="",suffix="",java.mem=10g){

## not.zero<-Control.alt.counts[,"Read.Balance"]!=0
## small<-Control.alt.counts[,"Read.Balance"]<0.01
## hist(Control.alt.counts[pass & small & not.zero,"Read.Balance"],breaks=50)

## sort(Control.alt.counts[pass,"Read.Balance"],decreasing=TRUE)[1:100]
## normal.allele.freq<-Control.alt.counts[test,"Read.Balance"]/100
## tumor.sample.major.counts<-normal.alt.counts[test,"REF.reads"]
## tumor.sample.minor.counts<-normal.alt.counts[test,"ALT.reads"]
## av.normal.in.tumor=1

poissonian.model.position.binom<-function( normal.allele.freq, tumor.sample.major.counts, tumor.sample.minor.counts, av.normal.in.tumor=1, lower.tail=FALSE){
...
# true.sample.major.counts+ true.sample.minor.counts
 ...
 # pred 30 1/1000=0.001 90% of bases btter than this in a typical run
 # -10*log10(0.001)
# -10*log10(0.00005)
 
#  normal.allele.freq[normal.allele.freq<0.00005]<-0.00005 ###Pred 43  histogram looks ok down to here set a minimum low allele freqency so maths does not go crazy
...
##  true.sample.minor.counts<-   8
 ...
 #p<-signif(2*(pnorm(z, mean = 0, sd = 1, lower.tail = lower.tail, log.p = FALSE)),digits=3)        ##this is the 2 tailed

 # p<-dbinom(as.integer(true.sample.minor.counts), as.integer(true.sample.cov), normal.allele.freq, log=FALSE)
   p<-pbinom(as.integer(true.sample.minor.counts), as.integer(true.sample.cov), normal.allele.freq,lower.tail = AD.lower.tail, log.p = FALSE)
 # ?dnbinom
...
poissonian.model.position<-function( normal.allele.freq, tumor.sample.major.counts, tumor.sample.minor.counts, av.normal.in.tumor=1, lower.tail=FALSE){
# av.normal.in.tumor =1 for no contaminaltion
...
#test<-c("chr17:18188827:18188827:A:G:snp","chr2:209113112:209113112:C:T:snp")

## test<-c("chrX:123195620:123195620:G:T:snp","chr2:209113113:209113113:G:A:snp:209113113")
## samples<-c("87","AMLM12003H-J","AMLM12003H-K","AMLM12015WPS","AMLM12021D-M","AMLM12027NM","AMLM12028DAK","AMLM12040A-J","AMLM12PAH037A-B")

## indels<-a.indel.stats[test,]
## the.samples<-samples
## normal.allele.freq<-Control.alt.counts[test,"Read.Balance"]/100
## alt.count.thresh<-2
## AD.extension<-"FAD"
...
genotype.p.values.row<-function(indels,the.samples,AD.extension="AD",normal.allele.freq,cellularity,alt.count.thresh=1,alt.count.thresh.include=2,AD.lower.tail = FALSE,Output.is.AD.corrected=FALSE){
...
#   print(AD.lower.tail)
  ...
  cellularity<-as.matrix(cellularity)
    if(dim(cellularity)[2]<2){print("ERROR dim(cellularity data matrix )<2 cellularity matrix must be col1=samples, col2=cellularity with 1=pure sample w.r.t Control")}

  if(dim(cellularity)[2]>2){print("Warning dim(cellularity data matrix )>2 cellularity matrix format must be col1=samples, col2=cellularity with 1=pure sample w.r.t Control")}
 ...
 #   print(paste("Cellualarity of",the.samples[missing],"missing-> using median"))
     print(paste("Cellualarity of",the.samples[missing],"missing-> make pure - assume a Control"))
 #   a.median<-median(as.numeric( cellularity[posns[!missing],2]),na.rm=TRUE) ## assumes first col is cellularity
  ...
  normal.allele.freq<-normal.allele.freq[posns]
   ...
   if(sum(missing)>0){print("Missing normal allel frequency: Error estimates of normal.all.freq missing set to 0.001")
  ...
  the.allele.counts<-apply(indels[,paste(the.samples,"GT",sep=".")],1,function(x) sum(x=="0/1" | x=="1/1"))
  

#  isnp<-2393
  if(!Output.is.AD.corrected){
 for(isnp in 1:length(rownames(p.matrix))){
 #   print(isnp)
   p.matrix[isnp,]<-poissonian.p.value.using.AD.row(indels[isnp,paste(the.samples,AD.extension,sep=".")],normal.allele.freq[isnp], cellularity[ cellularity[,1] %in% the.samples, 2],the.allele.counts[isnp] ,alt.count.thresh,alt.count.thresh.include,AD.lower.tail)
 ...
 for(isnp in 1:length(rownames(p.matrix))){
#   print(isnp)
   p.matrix[isnp,]<-poissonian.AD.corrected.row(indels[isnp,paste(the.samples,AD.extension,sep=".")],normal.allele.freq[isnp], cellularity[ cellularity[,1] %in% the.samples, 2],the.allele.counts[isnp],alt.count.thresh,alt.count.thresh.include,AD.lower.tail)
  #   p.matrix[,isample]<-poissonian.p.value.using.AD(subset(indels,select=paste(the.samples[isample],"AD",sep=".")),normal.allele.freq, cellularity[cellularity[,1]==the.samples[isample],2])
 }
}
  
#  indels[,paste(the.samples,AD.extension,sep=".")]
...
## isnp<-1
## genotypes<-indels[isnp,paste(the.samples,AD.extension,sep=".")]
## normal.allele.freq <- normal.allele.freq[isnp]
## av.normal.in.tumor<-cellularity[ cellularity[,1] %in% the.samples, 2]
## allele.counts<-the.allele.counts[isnp]
## alt.count.thresh<-1
## AD.lower.tail<-FALSE

poissonian.p.value.using.AD.row<-function(genotypes,normal.allele.freq,av.normal.in.tumor=1,allele.counts,alt.count.thresh=1,alt.count.thresh.include=2,AD.lower.tail = FALSE){

  ## if a genotypes has alt.count.thresh or less eg (10,1) then they will have p-vales set to zero unless the are at alt.count.thresh.include other samples or more of that type
##     print(alt.count.thresh)
##     print(av.normal.in.tumor)
## print(alt.count.thresh.include)
     default.seq.error.rate<-0.001

     
    genotypes<-as.character(genotypes) ## with rows will stay as a data frame
...
normal.allele.freq[ is.na(as.numeric(normal.allele.freq)) | !is.finite(as.numeric(normal.allele.freq))  ]<-default.seq.error.rate
...
# normal.allele.freq[normal.allele.freq<0.005]<-0.005 ### set a minimum low allele freqency so maths does not go crazy
 ...
 normal.allele.freq[normal.allele.freq < default.seq.error.rate]<-default.seq.error.rate ### set a minimum low allele freqency so maths does not go crazy
 true.sample.cov[true.sample.cov<=0]<-1  ## case when normal has coverge but tumor does not!
...
##      p<-dbinom(as.integer(true.sample.minor.counts), as.integer(true.sample.cov), normal.allele.freq, log=FALSE)
    ...
    ######## For a binomial test zero counts can  be significant
     p[as.integer(true.sample.minor.counts)==0]<-1

 #  samples.with.alt.bases<- sum(true.sample.minor.counts >= (1-default.seq.error.rate))
 # if(samples.with.alt.bases <= alt.count.thresh.include){
     
  if(allele.counts <= alt.count.thresh.include){
...
poissonian.AD.corrected.row<-function(genotypes,normal.allele.freq,av.normal.in.tumor=1,allele.counts,alt.count.thresh=1,alt.count.thresh.include=2,AD.lower.tail = FALSE){
...
#  p<-dbinom(as.integer(true.sample.minor.counts), as.integer(true.sample.cov), normal.allele.freq, log=FALSE)
     p<-apply(cbind(as.integer(true.sample.cov-true.sample.minor.counts),as.integer(true.sample.minor.counts)),1,function(x) paste(x,collapse=","))
 ...
 # if(samples.with.alt.bases <= alt.count.thresh.include){
  ...
  if(allele.counts <= alt.count.thresh.include){ 
  p[true.sample.minor.counts <= alt.count.thresh]<-genotypes[true.sample.minor.counts <= alt.count.thresh] # if  true.sample.minor.counts < alt.count.thresh set to not significant
                                                        }
 ## stats<-cbind(z,p)
 ## colnames(stats)<-c("Z-score","P-value") 
...
get.GERP.MULT<-function(db.details,indels,a.chunk=500){
  ...
  if(grepl("^chr",names(use.chrs)[1])){
    names(use.chrs)<-gsub("^chr","",names(use.chrs))
    indels[,"chr"]<-gsub("^chr","",indels[,"chr"])
  } ## gerp databse does not use chr
  ...
  for(ic in 1:length(use.chrs)){
   ...
   # result[[ic]]<-parallel(get.GERP.for.chromsome.MULTI(db.details,use.chr,the.starts,the.ends,a.insert,a.chunk))
     result[[ic]]<-get.GERP.for.chromsome.MULTI(db.details,use.chr,the.starts,the.ends,a.insert,a.chunk)
  }
  ...
  #  chr.gerp.data<-collect(result[[ic]],wait=TRUE)
    chr.gerp.data<-result[[ic]]
    ####Felicity: trying to fix gerp score issue
     gerp.data[to.get]<-chr.gerp.data
...
get.GERP.for.chromsome.MULTI<-function(db.details,use.chr,the.starts,the.ends,a.insert,a.chunk){
  ...
  ## db.details<-list(user="gerp2",pass="GeRpUQ8!6", dbname="gerp", host="ga-apps.di.uq.edu.au") #"di_mysql01.di.uq.edu.au"
  ## db.details<-list(user="gerp2",pass="GeRpUQ8!6", dbname="gerp", host="di-mysql01.di.uq.edu.au") 
  xx<-try(con <- dbConnect(drv,user=db.details$user,pass=db.details$pass, dbname=db.details$dbname, host=db.details$host),silent=TRUE)
  if(inherits(xx, "try-error")){
    print("WARNING Can't connect to gerp database, try second database")
    db.details<-list(user="gerp2",pass="GeRpUQ8!6", dbname="gerp", host="di-mysql01.di.uq.edu.au") 
    xx<-try(con <- dbConnect(drv,user=db.details$user,pass=db.details$pass, dbname=db.details$dbname, host=db.details$host),silent=TRUE)
    if(inherits(xx, "try-error")){
      print("WARNING Can't connect to ANY gerp database- ALL GERP set to 0")
    }else{
      print("Got connention to di-mysql01.di.uq.edu.au")
    }
  }
  ...
  if(length(the.starts)==0){
    print("Error in get.GERP.for.chromsome.MULTI no data to get" )
  }
  ...
  for(ichunk in 1:(length(the.seq)-1) ){
    ...
    ##     the.start
    ##     the.end
    if(ichunk%%(a.chunk*10)==0){
      print(paste("iter:",ichunk," region:",the.start,the.end))
    }
    ####
    ...
    # This seems to be the only query to the database:
    #
    # SELECT DISTINCT position, score 
    # FROM gerp 
    # WHERE chr = ? AND (position >= ? AND position <= ?)
    ...
    if(!inherits(xx, "try-error")){
      ...
      gerp.chr<-cbind(starts.chunk,0)
      colnames(gerp.chr)<-c("position","score")
    }
