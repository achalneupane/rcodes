...
#### abive codes is eacst record of analysis to this point.
#### Note a.indel.ori contains the case/ Normal genotype prior to genotype recovery
#### sample 27 has been removed
...
code.dir<-"/media/UQCCG/Programming/VersionControl_GitRepository/UQCCG_Pipeline_Rscripts"  ## directory where genotypes are located
felicities.filter.files.loc<-"/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Snp_read_filter" ## directory where SNP info data is located
cellularity.data.file<-"/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Sequenza/NSLM.cellularity.summary.use.csv"

setwd(code.dir)
source("annotate_SNPs_subroutines.r")
source("hwe.r")
load("Data.Freeze.LEO.RData")
load("a.indel.ori.LEO.RData")
load("SKAT_cohort_files_for_a.indel.ori.RData")
...
##########################################################################
#################
################ applify extra filtering from FELICITY

files<-dir(felicities.filter.files.loc)
...
pass.filt<-unlist(lapply(pass.filt,function(x) {sum(as.logical(x[1:3]))==0})) ## must pass first 3 filters
...
##########################################################################
##########################################################################
######################### Perfrom the snps weight model assume non-codng are benigh passemgers
##########################################################################
##########################################################################
##########################################################################
...
pass_coding<- full.qual &  bad.coding & maf.filter   & !in.common.hit.gene   & !unannotated.hits & not.flat.genotype  &
  ok.missing & hw.controls.ok.filt & !no.genotypes.filt &  rare.in.Control.filt & !in.any.normal.filt & !in.any.normal & pass.possonian.control.model & !bad.qual.locations
...
sort(table(a.indel[pass_coding,"Consequence.Embl"]))
sort(table(a.indel[pass_non.coding,"Consequence.Embl"]))
...
## tight.synon<-c("synonymous_variant","synonymous_variant,NMD_transcript_variant")
## tight.coding<-c("missense_variant","stop_gained","missense_variant,NMD_transcript_variant","missense_variant,splice_region_variant")
## synonymous<-test.for.coding.type(a.indel,geneanno.DB,synonymous.variant)
## sum(synonymous)

target<-modif & pass_coding
#target<-modif & pass_coding & (a.indel[,"Consequence.Embl"] %in% tight.coding) 
sort(tapply(a.indel[target ,"Gene.Names"],a.indel[target,"Gene.Names"],length),decreasing=TRUE)[1:10]

target.non<-modif & pass_non.coding
#target.non<-modif & pass_non.coding  & (a.indel[,"Consequence.Embl"] %in% tight.synon)
sort(tapply(a.indel[target.non,"Gene.Names"],a.indel[target.non,"Gene.Names"],length),decreasing=TRUE)[1:10]
# 
# sort(tapply(a.indel[pass_coding ,"Gene.Names"],a.indel[pass_coding,"Gene.Names"],length),decreasing=TRUE)[1:10]
# sort(tapply(a.indel[pass_non.coding,"Gene.Names"],a.indel[pass_non.coding,"Gene.Names"],length),decreasing=TRUE)[1:50]
grep("TTN",pass_non.coding_count)
...
write.table(weights,file="/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Felicity_filter/new_weights_final.txt",sep="\t",row.names=F,col.names=T
#write.table(weights,file="/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Felicity_filter/new_weights_final_USED.txt",sep="\t",row.names=F,col.names=T)
weights[1:5,]
...
## original.model.TTN<-use.wieght 
the.model
par(mfrow=c(1,1),font=2,font.lab=2,font.axis=2,mgp=c(3.5,1,0),mar=c(5,5,4,2)+0.4)
plot(as.numeric(weights[,2]) ,as.numeric(weights[,3]),main="For C>T motif SNPs",xlab="Non-Protein Coding SNPs",ylab="Protein Coding SNPs",cex.lab=2.0,cex.main=2.0,xlim=c(1,50))

plot(as.numeric(weights[,2]) ,as.numeric(weights[,3]),main="For C>T motif SNPs",xlab="Non-Protein Coding SNPs",ylab="Protein Coding SNPs",cex.lab=2.0,cex.main=2.0,xlim=c(5,90))
show<-identify(as.numeric(weights[,2]) ,as.numeric(weights[,3]),labels=weights[,1],atpen=TRUE,cex=1.25)
points(as.numeric(weights[show,2]) ,as.numeric(weights[show,3]),col="red",pch=20,cex=1)
...
the.model<-try(lmrob(NonSynonymous~Synonymous,data=weights,subset=c(1:dim(weights)[1])[weights$Synonymous >=count.threshold],silent=FALSE) )
               abline(coef(the.model),lty=10,col="red",lwd=2)

use.wieght<-coef(the.model)  ### set this as the model to use
 

the.model<-try(lmrob(NonSynonymous~Synonymous,data=weights,subset=c(1:dim(weights)[1])[(weights$Synonymous >=count.threshold) & !(weights$Gene %in% c("TTN"))],silent=FALSE) )
 abline(coef(the.model),lty=10,col="blue",lwd=2)

the.model<-try(lmrob(NonSynonymous~Synonymous,data=weights,subset=c(1:dim(weights)[1])[(weights$Synonymous >=count.threshold) & !(weights$Gene %in% c("TTN","MUC16"))],silent=FALSE) )
 abline(coef(the.model),lty=10,col="forestgreen",lwd=2) 
               



leg.txt<-c("Model A","Model B","Model B excl. TTN","Model B excl. TTN, MUC16")
legend(6,90,leg.txt,col=c("red","magenta","blue","forestgreen"),lty=c(2,2,2,2),pch=c(-1,-1,-1,-1),cex=1.5)
legend(6,200,leg.txt,col=c("red","magenta","blue","forestgreen"),lty=c(2,2,2,2),pch=c(-1,-1,-1,-1),cex=1.5)

setwd("/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Analysis/Figures")
fig.prefix<-"Q-Q_Final_6sd_A.NoRescue_B.Wrescue_TIGHT_SYNON"




savePlot(filename=paste("UV_wieght_model_final_TIGHT.png",sep=''),type="png")
savePlot(filename=paste("UV_wieght_model_final_TIGHT.jpeg",sep=''),type="jpeg")
savePlot(filename=paste("UV_wieght_model_final_TIGHT.tiff",sep=''),type="tiff")
#savePlot(filename=paste(fig.prefix,"aa.bmp",sep=''),type="bmp")
dev.print(svg,paste("UV_wieght_model_TIGHT..svg",sep=''))
getwd()
...
############################# RESCUE SOMATIC for low clonicity variants


## summary.geno.extra.ori<-summary.geno.extra
## a.indel.ori<-a.indel

cellularity<-read.delim(cellularity.data.file,header=T,sep="\t",fill=TRUE,skip=0,stringsAsFactors=FALSE)
...
geno.p<-genotype.p.values(a.indel.ori[test,],cancer,Control.alt.counts[test,"Read.Balance"]/100,cellularity)
...
geno.p<-genotype.p.values(a.indel.ori[snp.only & has.one.geno ,] ,c(cancer,PDs),Control.alt.counts[snp.only & has.one.geno,"Read.Balance"]/100,cellularity)
...
sum(!( colnames(geno.p) %in% colnames(a.indel.ori))) # must ge zero
to.transfer<-colnames(geno.p)[colnames(geno.p) %in% colnames(a.indel.ori)]

posns<-match(rownames(a.indel.ori),rownames(geno.p))
missing<-is.na(posns)
sum(missing)
...
snap.file
snap.file<-paste(snap.file,"QC_no27.FINAL_6sd_with Troels_wieights",sep="")
snap.file
...
## interesting.gene<-c("BCL2L12","CCDC61","STK19","KNSTRN","TRHDE","FREM2","EBNA1BP2","PHACTR3","DCLK1","LRRIQ1","PHACTR3","CSMD3") # hard code pass to get synonymous contribution
...
######################################################################################################
######################################################################################################
#################### FORCE GOOD CALLS BCLsynonymous  check KNSTRN  add back CDKN@A that failed missing filter
...
######################################################################################################
######################################################################################################
##############   Add weights to the SNP info matrix
...
# pass<-rep(TRUE,times=dim(a.indel)[1])
# genotypes<-a.indel[pass,the.samples.use.no27] ## ordered correctly for phenotypes
# genotypes<-a.indel.ori[pass,the.samples.use.no27] ## ordered correctly for phenotypes
...
## rownames(genotypes)<-gsub(".GT$","",the.samples.use.no27)



## # DONT't RUN these line DONE at UQCCG for all valiants (just subet below using snpinfo
##  if(target.pheno.col %in% case.control){
##  cohort.seq <- skatCohort(Z=genotypes,formula, SNPInfo = snpinfo, data=pheno,aggregateBy="cluster",family=binomial(),verbose=FALSE)
##     cohort.seq.ex <- skatCohort(Z=genotypes,formula, SNPInfo = snpinfo, data=pheno,aggregateBy="Name",family=binomial(),verbose=FALSE)


##   cohort.seq.ori <- skatCohort(Z=genotypes,formula, SNPInfo = snpinfo, data=pheno,aggregateBy="cluster",family=binomial(),verbose=FALSE)
##     cohort.seq.ori.ex <- skatCohort(Z=genotypes,formula, SNPInfo = snpinfo, data=pheno,aggregateBy="Name",family=binomial(),verbose=FALSE)
## save(list=c("cohort.seq.ori","cohort.seq.ori.ex"),file="SKAT_cohort_files_for_a.indel.ori.RData")
 
## }else{
## cohort.seq <- skatCohort(Z=genotypes,formula, SNPInfo = snpinfo, data=pheno,aggregateBy="cluster",family=gaussian(),verbose=FALSE) ## genes and clusters for 
## }
...
# snpinfo.19<-snpinfo[grepl("chr19",snpinfo[,"Name"]),]
...
paste("Burden","ALL",snap.file,"txt",sep=".") ## output file name
...
########
######## Choose A LIST OF GENEs to re convolve back to original


the.top<-550
to.unwind<-c(meta.results.burden[1:the.top,"gene"],meta.results.skatO[1:the.top,"gene"]) # this one does the top 50


#### is one to do 'specific' gene(s) then do below
...
## to.unwind<-c("BCL2L12","CCDC61","STK19","KNSTRN")


##################################set the prefix for the piutput file name
...
######################################## OUTPUT existing gens-wise data for these genes
the.genes
...
#################################### Now do the single point analysis using only 
sum(subset)
length(subset)
snpinfo[1:5,]
...
## meta.results.skat.ex<-skatMeta(cohort.seq,SNPInfo = snpinfo)  ### already calculated (and includes gene weights
...
genotypes.PD<-a.indel[figure, c("LPH-001-27_PD.GT",paste(pheno.ori[pheno.ori[,"PD"],"SAMPLE"],".GT",sep="")) ]  ## REPORT 27 samples that are positive (but not used in  the acciation test
genotypes.PD<-t(genotypes.PD)

genotypes.ex<-a.indel[figure,  unique(c("LPH-001-27_AK1.GT","LPH-001-27_AK2.GT","LPH-001-27_SCC.GT",paste(pheno.ori[pheno.ori[,"cancer"],"SAMPLE"],".GT",sep=""))) ]  ## REPORT 27 samples that are positive
...
muts.in.cases<-apply(genotypes.ex,2,function(x) { paste(names(x)[x!=0 & !is.na(x)],collapse=",")})
...
##########################################################################
colnames(summary.geno.extra)
muts.in.controls<-summary.geno.extra[figure,"GENO.Control"]
...
############# save a local copy of everything

#save.image("FINAL_with_wights_6sd_resure_UNK_18_APR.RData")
save.image("FINAL_with_wights_6sd_resure_UNK_23_APR.RData")

################# strip controls  for sharing
...
if(length(bad.samples.labels)>1){
a.indel<-a.indel[,colnames(a.indel)[!(colnames(a.indel) %in% bad.samples.labels)]]
a.indel.ori<-a.indel.ori[,colnames(a.indel.ori)[!(colnames(a.indel.ori) %in% bad.samples.labels)]]
genotypes<-genotypes[rownames(genotypes)[!(rownames(genotypes) %in% Control)],]
}



save.image("Data.Freeze.LEO.RData")
#######################################################################
#######################################################################
#######################################################################
#######################################################################
...
#
file<-"/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Analysis/Burden.ALL.coding.somatic.with.Indels.noBenign.UNK.wRepeatsQC_no27.FINAL_6sd.txt"
...
subset<-meta.results.burden[ !(meta.results.burden[,"gene"] %in%  clusters.wanted) ,] ## just plot SNPs not clusters
subset[1:10,]
...
z[!is.na(z)]
the.plot<-my.qq.plot(z,dist="chisq",df=1,ncp=0,col="blue",ylab="Observed chi-squared value",xlab="Expected chi-squared value",main="",cex=1,xlim=c(5,17.5),ylim=c(20,95),cex.lab=2.0,cex.axis=2.0,font.lab=2,font.axis=2,lwd=2,line="robust",plot.it=TRUE) # function defined below
...
leg.txt<-c(expression(paste("Genes")),"95% confidence intervals","Genes loss-of-function  mutations","Genes with gain-of-functions","Known SCC Genes")
legend(6,80,leg.txt,col=c("blue","red","magenta","forestgreen","red"),lty=c(-1,2,-1,-1,-1),pch=c(1,-1,19,19,25),cex=2.0)

setwd("/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Analysis/Figures")
fig.prefix<-"Q-Q_Final_6sd"
