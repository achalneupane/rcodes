...
library(RMySQL)
library(multicore)
library(foreach)  ## could not get1G this to work well
library(doMC)
...
## Mhairi - remove the UnAff (sporadiac) GT & AD columns
the.Unaffected.GT <- the.Unaffected
the.Unaffected.AD <- the.Unaffected
the.Unaffected.DP <- the.Unaffected
the.Unaffected.GT <- paste(the.Unaffected.GT, "GT",sep=".")
the.Unaffected.AD <- paste(the.Unaffected.GT, "AD",sep=".")
the.Unaffected.DP <- paste(the.Unaffected.DP, "AD",sep=".")
#:0(MAF).Q :0.001(MAF).Q :0.005(MAF).Q :0.01(MAF).Q  :0.025(MAF).Q  .DP
posns.unAff.GT <- colnames(summary) %in% the.Unaffected.GT
summary <- summary[,!posns.unAff.GT]

posns.unAff.AD <- colnames(summary) %in% the.Unaffected.AD
summary <- summary[,!posns.unAff.AD]

posns.unAff.DP<- colnames(summary) %in% the.Unaffected.DP
summary <- summary[,!posns.unAff.DP]
