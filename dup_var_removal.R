...
vcf <- read.delim(args[4], head=T,stringsAsFactors=F,comment.char ='#')
vcf_sub <- vcf[,-c(1,2,4:9)]
num_line <- nrow(vcf)
line=paste("read vcf file with ",num_line," lines.", sep="")
write(line,file="R_report_dup_var_removal.txt",append=TRUE)


combo <- merge(data_miss,vcf_sub, by.x = "SNP", by.y = "ID")
combo_order <- combo[order(combo$dummy_ID,combo$allele1, combo$allele2),]

line="Preparing a subset of data with only duplicate pairs and same call rate to plot the call correlation for those"
write(line,file="R_report_dup_var_removal.txt",append=TRUE)

#find and remove higher order dups
table_reps <- as.data.frame(table(combo$dummy_ID),stringsAsFactors=F)
...
#list_higher_order <- combo[combo$dummy_ID %in% higher_order$SNP,c(1:11)] 

plot_data <- combo[!combo$dummy_ID %in% higher_order$SNP,]
plot_data_order <- plot_data[order(plot_data$dummy_ID,plot_data$allele1, plot_data$allele2),]

##extract those dup var with the same call rate for plot
subset_plot <- plot_data_order[FALSE,]

for (i in 2:nrow(plot_data_order)){
  if (plot_data_order[i,7] == plot_data_order[(i-1),7] & plot_data_order[i,11] == plot_data_order[(i-1),11]) { 
    subset_plot[i,] <- plot_data_order[i,]
    subset_plot[i-1,] <- plot_data_order[i-1,]
  }
  else{subset_plot[i,] <- NA}
}


#keep row without NA
subset_plot_clean <- subset_plot %>% filter_all(any_vars(!is.na(.)))

#calculate call correlation for the subset
call_cor <- vector()
for (i in 2:nrow(subset_plot_clean)){
  call_cor[1] <- 0
  comp <- subset_plot_clean[i,12:ncol(subset_plot_clean)] == subset_plot_clean[i-1,12:ncol(subset_plot_clean)]
  mysum <- sum(comp)
  call_cor[i] <- (mysum*100)/(ncol(subset_plot_clean)-11)
}


subset_plot_clean2 <- subset_plot_clean[,-(12:ncol(subset_plot_clean))]
subset_plot_clean3 <- cbind(subset_plot_clean2,call_cor)

#only keep one row for plot (the even rows with correct call cor)
hist_prep = subset_plot_clean3[seq(2, nrow(subset_plot_clean3), by = 2), ]
...
### Now calculate the call cor for the complete dataset
# the correlation is calculated for a row with its top row
call_cor <- vector()
for (i in 2:nrow(combo_order)){
  call_cor[1] <- 0
  comp <- combo_order[i,12:ncol(combo_order)] == combo_order[i-1,12:ncol(combo_order)]
  mysum <- sum(comp)
  call_cor[i] <- (mysum*100)/(ncol(combo_order)-11)
}

##remove the calls and add this col
combo_order2 <- combo_order[,-(12:ncol(combo_order))]
combo_order3 <- cbind(combo_order2,call_cor)
...
for(i in 2:nrow(combo_order3)){
  ...
  if (combo_order3[i,7] == combo_order3[(i-1),7]) { # if the dummyIDs of two consecutive rows are the same and we know that none of the alleles are zero (previous conditional has filtered), then:
    ...
    { if (combo_order3[i,11] == combo_order3[(i-1),11] & combo_order3[(i),12] >= as.numeric(args[5])) # if none of the duplicates that have the same alleles (or RC), have any missng data, check the correlation between the calls. if they are more than 99% correlated, they are identical alleles, remove the ith row
    {no_dups[i,] <- NA}
      if (combo_order3[i,11] == combo_order3[(i-1),11] & combo_order3[(i),12] < as.numeric(args[5]) ) # if none of the duplicates that have the same alleles (or RC), have any missng data, check the correlation between the calls. if they are more than 99% correlated, they are identical alleles, remove the ith row
      {no_dups[i,] <- NA
       no_dups[(i-1),] <- NA}
...
for (i in 2:nrow(no_dups2)){
  ...
  if (no_dups2[i,7] == no_dups2[(i-1),7]) { # if the dummyIDs of two consecutive rows are the same and we know that none of the alleles are zero (previous conditional has filtered), then:
    ...
    { if (no_dups2[i,11] == no_dups2[(i-1),11] & no_dups2[(i),12] >= as.numeric(args[5])) # if none of the duplicates that have the same alleles (or RC), have any missng data, check the correlation between the calls. if they are more than 99% correlated, they are identical alleles, remove the ith row
    {no_dups3[i,] <- NA}
      if (no_dups2[i,11] == no_dups2[(i-1),11] & no_dups2[(i),12] < as.numeric(args[5])) # if none of the duplicates that have the same alleles (or RC), have any missng data, check the correlation between the calls. if they are more than 99% correlated, they are identical alleles, remove the ith row
      {no_dups3[i,] <- NA
      no_dups3[(i-1),] <- NA}
