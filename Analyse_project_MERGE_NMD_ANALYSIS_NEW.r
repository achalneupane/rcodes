...
table(sample.sheet.full[,"AffectionStatus"])
...
for(ichr in 3:length(project.files)){ ### loop over chromosomes
...
##  }
## clinical.genes[duplicated(clinical.genes)]

## last.cluster.length

## clusters.have<-clusters.wanted
## not.assigned<-unique(snpinfo[ !(snpinfo[,"cluster"] %in%  clusters.have)  ,"gene"])
## ic<-2

## while(length(not.assigned) > last.cluster.length){
##   cluster.genes<-sample(not.assigned,last.cluster.length,replace=FALSE)
##   last.cluster.length<-length(cluster.genes)
##   new.cluster<-paste("random",ic,sep=".")

##   snpinfo[ snpinfo[,"gene"] %in%  cluster.genes  ,"cluster"] <-new.cluster
##   clusters.have<-c(clusters.have,new.cluster)
##   not.assigned<-unique(snpinfo[ !(snpinfo[,"cluster"] %in%  clusters.have)  ,"gene"])
##   ic<-ic+1
##   print(ic)
##  }

## not.assigned<-!(snpinfo[,"cluster"] %in%  clusters.have)
## snpinfo[not.assigned,"cluster"]<-paste("random",ic,sep=".")
## clusters.have<-c(clusters.have,paste("random",ic,sep="."))

## sum(not.assigned)
## clusters.have[1:5]                     
## snpinfo[1:5,]
## /media/UQCCG/Software/annovar/humandb/Gene_symbol_aliases.txt
...
clusters[clusters[,1] %in% snpinfo[,"gene"],1]
clusters[clusters[,1] %in% snpinfo[,"gene"],2]
############################
...
the.projects<-names(table(pheno[,"SampleProject"]))
...
targets<-the.projects[!(the.projects %in% target)] #c("NMD","ex.Control","AOGC")
targets
...
hw.p.ex.control<-getHWE(summary.geno.extra[,"GENO.ex.Control"]) ## used 16 CPUs
hw.p.ex.control.filt<-getHWE(summary.geno.extra[,"GENO.ex.Control.filt"]) ## used 16 CPUs

hw.p.AOGC<-getHWE(summary.geno.extra[,"GENO.AOGC"]) ## used 16 CPUs
hw.p.AOGC.filt<-getHWE(summary.geno.extra[,"GENO.AOGC.filt"]) ## used 16 CPUs

#differ<-hw.p.control.filt != hw.p.ex.control.filt
#sum(differ)
...
names(hw.p.ex.control)<-key
names(hw.p.ex.control.filt)<-key
names(hw.p.AOGC)<-key
names(hw.p.AOGC.filt)<-key
...
group.maf.thresh<-0.10 # group.maf.thresh<-0.10  if more common that this in Group then discard: Discard Rare but common in this cohort ## not a point level
missing.threshold<-0.10 # missing.threshold<-0.50  60 % of genotypes missing
## missing.threshold.nimblgen<-0.20
## missing.threshold.illumina<-0.20 
c(the.projects,paste(targets,".filt",sep=""))
...
paste("MAF.",c(the.projects,paste(targets,".filt",sep="")),sep="")

      
rare.in.group<-combine.boolean(rare.in.group.table,c("MAF.NMD","MAF.Control"),"OR")
rare.in.group.filt<-combine.boolean(rare.in.group.table,c("MAF.NMD.filt","MAF.Control.filt"),"OR")
rare.in.group.controls<-combine.boolean(rare.in.group.table,c("MAF.AOGC","MAF.Control","MAF.ex.Control"),"AND")
rare.in.group.controls.filt<-combine.boolean(rare.in.group.table,c("MAF.AOGC.filt","MAF.Control.filt","MAF.ex.Control.filt"),"AND")


sum(!rare.in.group)
sum(!rare.in.group.filt)
sum(!rare.in.group.controls)
sum(!rare.in.group.controls.filt)
...
no.genotypes.table<-(summary.geno.extra[,c("MAF.NMD","MAF.Control","MAF.NMD.filt","MAF.Control.filt")]== 0) | summary.geno.extra[,c("MAF.NMD","MAF.Control","MAF.NMD.filt","MAF.Control.filt")]=="NaN"  | (is.na( summary.geno.extra[,c("MAF.NMD","MAF.Control","MAF.NMD.filt","MAF.Control.filt")])) # no genotypes in test classes for a mutataion after individaul quality filtering
no.genotypes.table[1:5,]
no.genotypes<-combine.boolean(no.genotypes.table,c("MAF.NMD","MAF.Control"),"AND") # was AND
no.genotypes.filt<-combine.boolean(no.genotypes.table,c("MAF.NMD.filt","MAF.Control.filt"),"AND") # was AND
sum(no.genotypes)
sum(no.genotypes.filt)

summary.geno.extra[1:5,]
missing.targets<-c(the.projects,paste(targets,".filt",sep=""))
high.missing<-{}
imt<-1
...
ok.missing.test[1:5,]
ok.missing<-combine.boolean(ok.missing.test,c("NMD","Control"),"AND") # was AND
ok.missing.filt<-combine.boolean(ok.missing.test,c("NMD.filt","Control.filt"),"AND") # was AND

ok.missing.ex<-combine.boolean(ok.missing.test,c("NMD","Control","ex.Control"),"AND") # was AND
ok.missing.filt.ex<-combine.boolean(ok.missing.test,c("NMD.filt","Control.filt","ex.Control"),"AND") # was AND


sum(!ok.missing)
sum(!ok.missing.filt)
sum(!ok.missing.ex)
sum(!ok.missing.filt.ex)
...
## table(a.indel[, "Gene.Biotype"])
## table(a.indel[,"gene_biotype"])
...
######################################
n<-max(as.integer(summary.geno.extra[,"TOTAL.Alleles.ex.Control"]))
n
alt.counts.thresh<-1
while( (alt.counts.thresh- n*p) / sqrt(n*p*(1-p)) <= sd.thresh){alt.counts.thresh<-alt.counts.thresh+1}
alt.counts.thresh
...
summary.geno.extra[1:5,]
rare.in.ex.controls<-as.numeric(summary.geno.extra[,"ALT.Alleles.ex.Control"])< alt.counts.thresh
rare.in.ex.controls.filt<-as.numeric(summary.geno.extra[,"ALT.Alleles.ex.Control.filt"])< alt.counts.thresh
sum(rare.in.ex.controls)
sum(rare.in.ex.controls.filt)
names(rare.in.ex.controls)<-key
names(rare.in.ex.controls.filt)<-key
##############################
...
hw.ex.controls.ok<-hw.p.ex.control > hwe.control.threshold
hw.ex.controls.ok.filt<-hw.p.ex.control.filt > hwe.control.threshold

hw.AOGC.ok<-hw.p.AOGC > hwe.control.threshold
hw.AOGC.ok.filt<-hw.p.AOGC.filt > hwe.control.threshold
## sum(hw.controls.ok)
## sum(hw.controls.ok.filt)
## sum(hw.ex.controls.ok)
## sum(hw.ex.controls.ok.filt)
## sum(!hw.controls.ok)
...
## !on.x.y  ## need for clusters

## pass<- full.qual &  bad.coding & maf.filter   & !in.common.hit.gene   & !unannotated.hits & not.flat.genotype & !are.repeats & !are.in.repeats &
## ( ok.missing.filt | is.unwound.geno)   & hw.controls.ok.filt & !no.genotypes.filt   & rare.in.controls.filt & rare.in.group & rare.in.AOGC & rare.in.AOGC.filt &
##  rare.in.ex.controls.filt  & hw.ex.controls.ok.filt & ( ok.missing.filt.ex | is.unwound.geno) ## was using this in first draft - is unwould geno is a problem I think
...
( ok.missing.filt )   & hw.controls.ok.filt & hw.AOGC.ok & hw.AOGC.ok.filt & !no.genotypes.filt   & rare.in.controls.filt & rare.in.group & rare.in.AOGC & rare.in.AOGC.filt &
 rare.in.ex.controls.filt  & hw.ex.controls.ok.filt & ( ok.missing.filt.ex )

 help<-cbind(full.qual,bad.coding,maf.filter,no.genotypes,in.common.hit.gene,hw.controls.ok,on.x.y,unannotated.hits,not.flat.genotype,are.repeats,are.in.repeats,ok.missing,ok.missing.filt,ok.missing.ex,ok.missing.filt.ex,is.unwound.geno,ok.missing.filt, is.unwound.geno,hw.p.control.filt,hw.controls.ok.filt,hw.AOGC.ok,no.genotypes.filt,rare.in.group,rare.in.group.filt,rare.in.controls,rare.in.controls.filt,rare.in.AOGC,rare.in.AOGC.filt)
...
pheno[1:5,]
table(pheno[,target.pheno])

pheno.use<-pheno[pheno[,target.pheno] %in% c(0,1),]
dim(pheno.use)
the.samples.use<-pheno.use[,"SAMPLE"]
the.samples.use<-paste(the.samples.use,".GT",sep="")


################

## fil.genotypes<-foreach(a.indel.bit=iter(a.indel[pass,],by='row',chunksize=as.integer(sum(pass)[1]/num.bits)), .combine='rbind', .multicombine=TRUE, .inorder=TRUE) %dopar% filtered.genotype(a.indel.bit,gsub(".GT$","",the.samples),prefix="",suffix="",20,0.2,0.80,0.05,0.95,10,5)
...
genotypes<-a.indel[pass,the.samples.use] ## ordered correctly for phenotypes and have phenotypes
...
cohort.seq <- skatCohort(Z=genotypes,formula, SNPInfo = snpinfo, data=pheno.use,aggregateBy="cluster",family=binomial(),verbose=FALSE) ## genes and clusters
...
save(list=c("meta.results.skat","meta.results.skatO","meta.results.burden","pheno.use","snpinfo","genotypes","pass","high.missing","a.indel","help","key","summary.geno.extra"),file=paste(paste(project.files[ichr],".small.RData",sep="")) )
print(paste("Done: ",project.files[ichr]))q()
...
##Analyse_project_collect_results.r
...
test<-c("FANCP")
test<-c("IDH1")
test<-c("NOTCH1") # not sig after coverage filtering at 0.01 0.1997688 from 0.000142
test<-clinical.genes
test<-fanc.genes
...
to.unwind<-c("APOBEC3G") # to.unwind<-meta.results.burden[8,"gene"]
...
out<-cbind(meta.results.burden.ex,annotations[figure,],summary.geno.extra[figure,c("GENO.AML","GENO.Control","GENO.AML.filt","GENO.Control.filt")],help[figure,],muts.in.cases,muts.in.controls)

out<-cbind(meta.results.burden.ex,annotations[figure,],help[figure,],muts.in.cases,muts.in.controls)
