...
wantID <- read.table("/40/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/06-Aquilla_202101-a/FASe_ID_list_round1.txt", header = FALSE, sep =",", check.names = FALSE)
head(wantID)
...
################################################################################################
# https://genome.sph.umich.edu/wiki/Code_Sample:_Generating_QQ_Plots_in_R
setwd("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/06-Aquilla_202101-a/01-Aquilla-preQC/03-PLINK-QC-files2/")
library(lattice)
...
SORTED_FEATURE <- TopFeatures[with(TopFeatures, order(P)), ]
SORTED_FEATURE$OR <- exp(SORTED_FEATURE$BETA)

SORTED_FEATURE <- SORTED_FEATURE[1:10,c("Variant_full", "Var_Type", "Gene", "MAF", "OR", "P") ]
write.table(SORTED_FEATURE, "Annotated_TOP_10_GENES_FASe_1502_from_Aquilla_after_QC-biallelic_FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_with_no_chr_epacts_single_variant_cov_SEX-PC1-PC2_PC3.csv", sep = ",", quote = FALSE, row.names = FALSE)
...
####
dd <- LOGISTIC.t[c(20,19,16,14,13,12,11,10)]
dd$oddsRatio <- exp(dd$SEBETA)
...
################################## FarVat ######################################
################################################################################
setwd("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/06-Aquilla_202101-a/01-Aquilla-preQC/03-PLINK-QC-files2/FarVat/")
...
SORTED_FEATURE <- LOGISTIC.t[with(LOGISTIC.t, order(P)), ]

SORTED_FEATURE <- SORTED_FEATURE[1:10,c("VarPos", "GENE", "P") ]
write.table(SORTED_FEATURE, "Annotated_TOP_10_GENES_FASe_1502_from_Aquilla_after_QC-biallelic_FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_with_no_chr_gene-based_CADD20_cov_SEX-PC1-PC2_PC3.csv", sep = ",", quote = FALSE, row.names = FALSE)
...
# HAPMAP
setwd("/40/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/01-Aquilla-preQC/03-PLINK-QC-files3")
PCA <- read.table("FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic_post_QC_with_STATUS-hwe-geno0.05-mind0.1-HAPMAP-MERGED3-for_PCA.eigenvec", header =T, stringsAsFactors=FALSE)
...
####
PCA <- read.table("FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic_post_QC_with_STATUS-hwe-geno0.05-mind0.1_post_QC2-HAPMAP-MERGED3-for_PCA.eigenvec", header =T, stringsAsFactors=FALSE)
HAPMAP.ethnicty <- read.table("relationships_w_pops_121708.txt", header = T )
head(HAPMAP.ethnicty)
