...
posns<-match(a.chk,rownames(a.indel))
pass_non.coding[posns]
in.any.normal[posns]

a.indel[chk,c(core.ann,"refGene::location","knownGene::location","ensGene::location", "Consequence.Embl" )]
a.indel[chk,1:70][2:3,]
a.indel[chk,c(core.ann,"refGene::location","knownGene::location",
              "ensGene::location", "Consequence.Embl","MAF.lt:0","MAF.lt:0.001","MAF.lt:0.005","MAF.lt:0.01","MAF.lt:0.025" )][1:3,]
summary.geno.extra[a.chk,]

mafs<-colnames(a.indel)[grepl("maf",colnames(a.indel))]

a.indel[chk,c(core.ann,"refGene::location","knownGene::location",
              "ensGene::location", "Consequence.Embl","MAF.lt:0","MAF.lt:0.001","MAF.lt:0.005","MAF.lt:0.01","MAF.lt:0.025",mafs )][1:3,]
...
write.table(weights,file="/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Felicity_filter/new_weights_final_MARIA_w_zeros.txt",sep="\t",row.names=F,col.names=T)
weights[1:5,]
...
plot(as.numeric(weights[,2]) ,as.numeric(weights[,3]),main="For C>T motif SNPs",xlab="Non-Protein Coding",ylab="Protein Coding",cex.lab=1.5)
identify(as.numeric(weights[,2]) ,as.numeric(weights[,3]),labels=weights[,1],atpen=TRUE)

par(mar=c(5.5,5.5,2,2),mgp=c(3,1,0))
plot(as.numeric(weights[,2]) ,as.numeric(weights[,3]),main="For C>T motif SNPs",xlab="Synonymous Counts",ylab="Non-synonymous Counts",cex.main=2.5,cex.lab=2.5,cex.axis=2.5,font=2,font.lab=2)
...
the.model<-try(lmrob(NonSynonymous~Synonymous,data=weights,subset=c(1:dim(weights)[1])[(weights$Synonymous >=10) & !(weights$Gene %in% c("TTN","MUC16"))],silent=FALSE) )
 abline(coef(the.model),lty=10,col="forestgreen",lwd=2) 
               
leg.txt<-c("Fit A","Fit B","Fit B excl TTN","Fit B excl TTN, MUC16")
legend(10,200,leg.txt,col=c("red","purple","blue","forestgreen"),lty=c(2,2,2,2),pch=c(-1,-1,-1,-1),cex=2.0,lwd=2)

getwd()
savePlot(filename=paste("UV_wieght_model_report.png",sep=''),type="png")
savePlot(filename=paste("UV_wieght_model_report.jpeg",sep=''),type="jpeg")
savePlot(filename=paste("UV_wieght_model_report.tiff",sep=''),type="tiff")
...
the.weights<-the.weights[!too.few.mutations]
...
test<-c("chr5:138268367:138268367:T:A:snp") 
...
snap.file<-paste(snap.file,".withGQ.filter.QCed_weights_TIGHT",sep="")
snap.file
...
## geno.p<-genotype.p.values(a.indel[pass & snp.only & has.one.geno ,] ,cancer,Control.alt.counts[pass,"Read.Balance"]/100,cellularity)
...
## sum(!( colnames(geno.p) %in% colnames(genotypes))) # must ge zero
## to.transfer<-colnames(geno.p)[colnames(geno.p) %in% colnames(genotypes)]

## posns<-match(rownames(genotypes),rownames(geno.p))
## missing<-is.na(posns)

## genotypes[!missing,to.transfer]<-geno.p[posns[!missing],to.transfer]
###################################################
...
load("/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Analysis/FINAL_with_wights_6sd_resure_UNK_19_MAY.RData")
setwd("/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Analysis")
###############$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
###############$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
###############$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
...
known<-meta.results.burden.ori[meta.results.burden.ori[,"p"]< 0.05/length(z) & !is.na(meta.results.burden.ori[,"p"]),"gene"]
have<-meta.results.burden[meta.results.burden[,"p"]< 0.05/length(z) & !is.na(meta.results.burden[,"p"]),"gene"]
...
## ## points(qq$x[show],qq$y[show],col=c("forestgreen","magenta"),pch=20,cex=2)
## #points(qq$x[show],qq$y[show],col=c("cyan"),pch=20,cex=2)
## #text(qq$x[show],qq$y[show],labels=symbols[qq$ord[show]],col=c("forestgreen","magenta"),pos=4,offset=1,cex=1.3)  #,pch=25## selected.data<-identify(qq$x,qq$y,labels=symbols[qq$ord],col="forestgreen",cex=1.3,offset=2,pos=4,atpen='TRUE')
...
length(z)
abline(h=qchisq(0.05/length(z),df=1,ncp=0,lower.tail=FALSE,log.p=FALSE),col="green",lw=2)
...
bad<-c( "ATN1","NUDT18","OR5D13","MUC4","OR9G4","OR8H3","ANKRD30BL,MIR663B","BAD")
...
have<-meta.results.burden[meta.results.burden[,"p"]< 2.537e-6 & !is.na(meta.results.burden[,"p"]),"the.gene"]
...
###### supplement
file.genes <- "/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Analysis/Melanoma/TCGA_mutSig.csv"

file.genes <- "/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Analysis/Melanoma/Malanomia_intogen.csv"
slide.hits<-read.delim(file.genes,header=T,skip=1,sep="\t",fill=TRUE,stringsAsFactors=FALSE)
slide.hits[1:5,]
slide.hits<-slide.hits[,"gene"]
slide.hits<-unqiue(slide.hits)
slide.hits

slide.hits<-c( "INTS1","NFKBIE","PRB3","TNXB","CDH11","CHST4","XIRP2","DOCK2")
...
dim(freq)
colnames(extra.all)<-colnames(freq)
freq
extra.all
...
length(as.character(names(labels)))
#fig.prefix<-"barplot_SCCAK_noNames"
fig.prefix<-"barplot_SCCAK_Names"
fig.prefix<-"barplot_PD_melanoma_Names"
getwd()
setwd("/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Analysis/Final_results")
setwd("/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Analysis/Melanoma")
savePlot(filename=paste(fig.prefix,".png",sep=''),type="png")
savePlot(filename=paste(fig.prefix,".jpeg",sep=''),type="jpeg")
savePlot(filename=paste(fig.prefix,".tiff",sep=''),type="tiff")
#savePlot(filename=paste(fig.prefix,".bmp",sep=''),type="bmp")
