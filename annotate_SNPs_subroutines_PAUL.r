...
## indels<-a.indel[pass,]
## the.samples<-cancer
## normal.allele.freq<-Control.alt.counts[pass,"Read.Balance"]/100
...
genotype.p.values<-function(indels,the.samples,normal.allele.freq,cellularity){
  ...
  cellularity[is.na(as.numeric(cellularity[,2])),2]<-median(as.numeric( cellularity[posns[!missing],2]),na.rm=TRUE)
  ...
  #  print(isample)
   p.matrix[,isample]<-poissonian.p.value.using.AD(indels[,paste(the.samples[isample],"AD",sep=".")],normal.allele.freq, cellularity[cellularity[,1]==the.samples[isample],2])
 }
...
##   allele.depths.group<-indels[,paste(the.samples,"AD",sep=".")]
##   allele.depths.group[genotypes=="0/0"]
##    signif(p.matrix[genotypes=="0/0"],digits=2)
##  allele.depths.group[p.matrix<  0.0026]
## p.matrix[p.matrix< 0.0026]
  
##    allele.depths.group[genotypes!="0/0"]
##    signif(p.matrix[genotypes!="0/0"],digits=2)
##   0.0026

##     2*(pnorm(abs(c(1:8)), mean = 0, sd = 1, lower.tail = FALSE, log.p = FALSE))
  
##  indels[,paste(the.samples[isample],"AD",sep=".")],normal.allele.freq, cellularity[cellularity[,1]==the.samples[isample],2]
## genotypes<-indels[,paste(the.samples[isample],"AD",sep=".")]
##   normal.allele.freq
## av.normal.in.tumor<-  cellularity[cellularity[,1]==the.samples[isample],2]
## indels[,paste(the.samples[isample],"GT",sep=".")]
  
poissonian.p.value.using.AD<-function(genotypes,normal.allele.freq,cellularity,av.normal.in.tumor=1){
#  print(genotypes)
...
##  true.sample.minor.counts<-   8
 ...
 z<- (true.sample.minor.counts- true.sample.cov*normal.allele.freq) / (sqrt(true.sample.cov*normal.allele.freq*(1-normal.allele.freq)))
 p<-2*(pnorm(z, mean = 0, sd = 1, lower.tail = FALSE, log.p = FALSE))        ##this is the 2 tailed
