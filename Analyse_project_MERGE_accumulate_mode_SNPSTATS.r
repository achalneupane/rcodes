...
###############################################
#/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-06-03_LeoPharma_NovoAlign/snpstats/2015-06-03_LeoPharma_NovoAlign.BEST.chrALL.ACC_0.025.ALL.ALL_GENOTYPES_analysis.txt_snpstats.chrX.stats.txt



UQCCG.data<-"/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/"
project<-"2015-06-03_LeoPharma_NovoAlign"
project.name <- "2015-06-03_LeoPharma_NovoAlign.BEST.chrALL.ACC_0.025.ALL.ALL_GENOTYPES_analysis.txt_snpstats."
#project.name <- project
annotate.dir <-paste(UQCCG.data,project,"Annotate",sep="/")
analysis.dir<-paste(UQCCG.data,project,"snpstats",sep="/")


project.extension<-"stats.txt" ; skip.lines<-8; sample.extension<-"FAD" ## for stats
project.extension<-"read.filter.txt"; skip.lines<-5; sample.extension<-"RF" ## for read.filter
project.extension<-"mnp.txt"; skip.lines<-8; sample.extension<-"RF" ## for read.filter

## project.extension<-"analysis-maf-filtered.txt_snpstats.stats.txt"
## project.extension<-"analysis-maf-filtered.txt_snpstats.read.filter.txt"
...
#  project.files<-project.files[!grepl("chrALL",project.files)] ## remove old combined runs
  project.files<-project.files[grepl(project.name,project.files)] ## remove old combined runs 
  ...
  for(ichr in 1:length(project.files)){
    ...
    ################## fast read ###########
    column.labels<-read.delim(project.files[ichr],header=F,skip=skip.lines,nrows=1,sep="\t",fill=TRUE,stringsAsFactors=FALSE,na.strings="",quote="\"")
    num.vars<-dim(column.labels)[2]
    a.indel<-scan(project.files[ichr],what=character(num.vars),skip=skip.lines+1,sep="\t",fill=TRUE,na.strings="",quote="") # quote="\""
    ...
    ########################################
    #a.indel<-read.delim(project.files[ichr],header=T,skip=skip.lines,fill=TRUE,sep="\t",stringsAsFactors=FALSE,check.names=FALSE)
    all.possible.samples<-gsub(paste0(sample.extension,"$"),"",colnames(a.indel)[grep(paste0(sample.extension,"$"),colnames(a.indel))],perl=TRUE)
   ...
   #   best.quality<-as.numeric(a.indel[,"GQ_MEAN"])>=GQ.thresh & a.indel[,"FILTER"]=="PASS"
     # best.quality<- a.indel[, "FILTER"]=="PASS" #  &  a.indel[,"wanted.muts"]=="TRUE" #  & a.indel[,"MAF.lt:0.005"]=="TRUE"
      ...
      #  best.quality2<-a.indel[,"Gene.Names"] %in%  wanted.genes
        best.quality2<-rep(TRUE,times=dim(a.indel)[1])
        sum(best.quality2)
...
#       write.table(a.indel[best.quality,],file=paste(project.name,"HC.2.BEST.chrALL.ACC_0.025",fam[ifam],project.extension,sep=""),col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)
       write.table(a.indel[best.quality2,],file=paste(project.name,"chrALL",fam[ifam],project.extension,sep=""),col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)
 ...
 #      write.table(a.indel[best.quality,],file=paste(project.name,"HC.2.BEST.chrALL.ACC_0.025",fam[ifam],project.extension,sep=""),col.names=FALSE,row.names=FALSE,sep="\t",quote=FALSE,append=TRUE)
       write.table(a.indel[best.quality2,],file=paste(project.name,"chrALL",fam[ifam],project.extension,sep=""),col.names=FALSE,row.names=FALSE,sep="\t",quote=FALSE,append=TRUE)
