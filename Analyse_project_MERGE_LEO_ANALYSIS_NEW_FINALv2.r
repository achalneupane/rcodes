...
pass_coding<- full.qual &  bad.coding & maf.filter   & !in.common.hit.gene   & !unannotated.hits & not.flat.genotype  & 
  ok.missing & hw.controls.ok.filt & !no.genotypes.filt &  rare.in.Control.filt & !in.any.normal.filt & !in.any.normal & pass.possonian.control.model & !bad.qual.locations & the.coding.snps
...
a.indel[chk,c("chr","start","end","refGene::location","knownGene::location","ensGene::location","Consequence.Embl")][1:50,]
...
write.table(weights,file="/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/Felicity_filter/new_weights_final_MARIA.txt",sep="\t",row.names=F,col.names=T)
weights[1:5,]
