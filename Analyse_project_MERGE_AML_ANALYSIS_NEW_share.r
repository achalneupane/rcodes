...
common.hit.genes<-names(all.genes)[1:10] # common.hit.genes<-names(all.genes)[1:4]
all.genes["SCN2A"]
...
clusters<-read.delim("/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2013-10-27_AML_with_AOGCControl_NoFailedLane/Analysis/Final_FANC_clusters.csv",header=T,sep="\t",fill=TRUE,stringsAsFactors=FALSE)
clusters
...
pass<- full.qual &  bad.coding & maf.filter   & !in.common.hit.gene  & !on.x.y & !unannotated.hits & not.flat.genotype & !are.repeats & !are.in.repeats &
ok.missing.filt & hw.controls.ok.filt & !no.genotypes.filt & rare.in.controls.filt & rare.in.group
...
save(list=c("clusters.wanted"),file="clusters.wanted.RData") # load(
getwd()
