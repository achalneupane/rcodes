...
## gt.labels<-expand.labels.to.samples(c("GT"),all.possible.samples)
## gq.labels<-expand.labels.to.samples(c("GQ"),all.possible.samples)
## i<-200
## for (i in 1:length(gt.labels)){

## has.geno<-a.indel[,"FILTER"]=="PASS" & a.indel[,"chr"]=="chr10" &  (a.indel[,gt.labels[i]] %in% c("0/1","1/1") & !is.na(a.indel[,gt.labels[i]])) & (as.numeric(a.indel[,gq.labels[i]])>20 & !is.na(a.indel[,gt.labels[i]]))
## sum(has.geno)
## sort(table(a.indel[has.geno,  "Consequence.Embl" ]))
## sort(table(a.indel[has.geno,  "knownGene::location" ]))
   ...
   #   best.quality<-as.numeric(a.indel[,"GQ_MEAN"])>=GQ.thresh & a.indel[,"FILTER"]=="PASS"
    ...
    #   best.quality<- a.indel[,"wanted.muts"]=="TRUE" 
 ...
 ##      dim(a.indel[an.indel,])
       ...
       if(ichr==1){
       write.table(a.indel,file=paste(project.name,".chrALL.ACC",fam[ifam],project.extension,sep=""),col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)
       write.table(a.indel[best.quality,],file=paste(project.name,".chrALL.ACC_wanted",fam[ifam],project.extension,sep=""),col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)
       ...
       write.table(a.indel,file=paste(project.name,".chrALL.ACC",fam[ifam],project.extension,sep=""),col.names=FALSE,row.names=FALSE,sep="\t",quote=FALSE,append=TRUE)
       write.table(a.indel[best.quality,],file=paste(project.name,".chrALL.ACC_wanted",fam[ifam],project.extension,sep=""),col.names=FALSE,row.names=FALSE,sep="\t",quote=FALSE,append=TRUE)
