...
# https://bioconductor.org/packages/release/bioc/vignettes/GENESIS/inst/doc/assoc_test_seq.html
...
GDSFILE="/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/06-Aquilla_202101-a/01-Aquilla-preQC/03-PLINK-QC-files4/GENESIS/gdsfile"

## seqVCF2GDS(vcffile,"/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/06-Aquilla_202101-a/01-Aquilla-preQC/03-PLINK-QC-files4/GENESIS/gdsfile", storage.option="ZIP_RA", parallel = 10L)
# seqVCF2GDS(vcffile, "/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/06-Aquilla_202101-a/01-Aquilla-preQC/03-PLINK-QC-files4/GENESIS//gdsfile", verbose=TRUE, parallel = 10L)
gds <- seqOpen("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/06-Aquilla_202101-a/01-Aquilla-preQC/03-PLINK-QC-files4/GENESIS/gdsfile")
gds
...
head(variant.id <- seqGetData(gds, "annotation/id"))
# [1] "1:69511:A:G"  "1:69536:C:A"  "1:69536:C:T"  "1:785910:G:C" "1:785911:C:A"
# [6] "1:785979:C:T"
...
sum(samp.id %in% PROJECTinfo$SM)

FAM <- cbind(samp.id, PROJECTinfo[match(samp.id, PROJECTinfo$SM), ])
...
FAM$IDs.with.no.Rep <- gsub("_R1$|_R2$", "", samp.id)
...
sum(FAM$samp.id == FAM$`Aquilla vcfID`)
sum(FAM$samp.id == as.character(FAM$SM))

df2 <- cbind.data.frame(IID=FAM$samp.id, Population=as.character(FAM$`ETHNICITY - Aurora`), Sex=FAM$Sex, STATUS=FAM$STATUS)

write.table(df2, "FASe_3894_Ethnicity_from_Aurora.txt", sep ="\t", col.names = T, quote = F, row.names = FALSE)
...
#############################Subset GDS analysis ####################################
# SNPID="/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP.bim"
# SAMID="/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP.fam"

SNPID="/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP-PCAS_genesis.bim"
SAMID="/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP-PCAS_genesis.fam"

# After QC
SNPID="/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP_post_QC2-PCAS_genesis.bim"
SAMID="/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP_post_QC2-PCAS_genesis.fam"

snpID <- read.table(SNPID, sep = "\t", header = F)
snpID$V2 <- as.character(snpID$V2)
sampleID <- read.table(SAMID, sep = " ", header = F)
sampleID$V2 <- as.character(sampleID$V2)

sum(snpID$V2 %in% variant.id)

snp.id <- match(snpID$V2, variant.id)
sample.id <- sampleID$V2

## LD pruning to get variant set using list of SNPs instead
# snpset <- snpgdsLDpruning(gds, method="corr", slide.max.bp=10e6, 
#                           ld.threshold=sqrt(0.1), num.thread=10L, verbose=TRUE)
# pruned <- unlist(snpset, use.names=FALSE)


king <- snpgdsIBDKING(gds, snp.id=snp.id, sample.id=sample.id, verbose=FALSE, num.thread = 10L)
## Or also with maf and missing.rate
king <- snpgdsIBDKING(gds, snp.id=snp.id, sample.id=sample.id, verbose=FALSE,  maf=0.01, missing.rate=0.01, num.thread = 10L)
...
# from selected SNPS and samples
...
pc.df$Population[is.na(pc.df$Population)] <- -9
unique(pc.df$Population)
...
pop.cols <- setNames(brewer.pal(6, "Paired"), c("EUR-LIKE", "-9", "HISP-LIKE",  "YOR_LIKE", "ASI", "ASI-tendency"))


# jpeg("GENESIS_FASe_1951_PC1_PC2_ggplot.jpeg", width = 1000, height = 1000, units = "px", pointsize = 16, quality = 75, bg = "white")
# ggplot(pc.df, aes(PC1, PC2, color=Population)) + geom_point() +
#   scale_color_manual(values=pop.cols)
# dev.off()
# jpeg("PCAIR_FASe_3982_PC1_PC2_ggplot.jpeg", width = 1000, height = 1000, units = "px", pointsize = 16, quality = 75, bg = "white")
# ggplot(pc.df, aes(PC1, PC2, color=Population)) + geom_point() 
# dev.off()
...
ggplot(pc.df, aes(PC1, PC2, color=Population)) + geom_point() +
    scale_color_manual(values=pop.cols)
ggsave("GENESIS_FASe_1803_PC1_PC2_ggplot_select_samples_select_variants_filtered_SNPs_from_Plink.jpeg", plot = last_plot(), device = NULL, scale = 1, width = 9, height = 9, dpi = 300, limitsize = TRUE)
# ggsave("GENESIS_FASe_1803_PC1_PC2_ggplot_select_samples_select_variants_maf_0.01_geno_0.01.jpeg", plot = last_plot(), device = NULL, scale = 1, width = 9, height = 9, dpi = 300, limitsize = TRUE)


## Correlation between GENESIS and PLINK
# Before QC
write.table(pc.df,"pc.df_GENESIS__select_samples_select_variants_filtered_SNPs_from_Plink_prior_QC.csv", sep =",", row.names = FALSE, col.names = T, quote = F)
write.table(PCAs,"PCAs_from_plink_for_GENESIS_correlation_prior_QC.csv", sep =",", row.names = FALSE, col.names = T, quote = F)
pc.df <- read.table("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/06-Aquilla_202101-a/01-Aquilla-preQC/03-PLINK-QC-files4/GENESIS/pc.df_GENESIS__select_samples_select_variants_filtered_SNPs_from_Plink_prior_QC.csv", sep =",", header = T)
PCAs <- read.table("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/PCAs_from_plink_for_GENESIS_correlation_prior_QC.csv", header = T, sep = ",")


# POST QC
write.table(pc.df,"pc.df_GENESIS__select_samples_select_variants_filtered_SNPs_from_Plink.csv", sep =",", row.names = FALSE, col.names = T, quote = F)
write.table(PCAs,"PCAs_from_plink_for_GENESIS_correlation.csv", sep =",", row.names = FALSE, col.names = T, quote = F)
pc.df <- read.table("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/06-Aquilla_202101-a/01-Aquilla-preQC/03-PLINK-QC-files4/GENESIS/pc.df_GENESIS__select_samples_select_variants_filtered_SNPs_from_Plink.csv", sep =",", header = T)
PCAs <- read.table("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/PCAs_from_plink_for_GENESIS_correlation.csv", header = T, sep = ",")


PLINK <- PCAs
GENESIS <- pc.df
colnames(pc.df) <- paste0(colnames(pc.df),"_GENESIS")
colnames(PCAs) <- paste0(colnames(PCAs),"_PLINK")
df.all <- merge(PCAs, pc.df, by.x = "IID_PLINK", by.y = "sample.id_GENESIS")
# TARGET <- as.character(PCAs$IID)
# df <- df[match(TARGET, df$IID),]
# PC <- cbind.data.frame(IID=df$IID, PLINK=df$PC1, GENESIS=df$PC1_GENESIS)
# V1 <- paste0(PC$IID, "_PLINK")
# V2 <- paste0(PC$IID, "_GENESIS")
# TARGET <- c(rbind(V1, V2))
# PC <- reshape2::melt(PC1)
# PC$Key <- paste(PC$IID, PC$variable, sep ="_")
# PC <- PC[match(TARGET, PC$Key),]
# head(PC)
# library(ggplot2)
# PC$Key <- as.factor(PC$Key)
# PC$variable <- as.character(PC$variable)
# df <- cbind.data.frame(x=PC$Key,y= PC$value)
...
PC_TYPE <- ALL[i]
VARX <- paste0(PC_TYPE, "_PLINK")
VARY <- paste0(PC_TYPE, "_GENESIS")
df <- df.all[!df.all$IID_PLINK %in% remove.SAM,]
# ## remove any outliers prior QC
# if (PC_TYPE == "PC3"){
# df <- df[(!grepl ("27_131", df$IID_PLINK)),]
# } else if (PC_TYPE == "PC4"){
# df <- df[(!grepl ("25_48", df$IID_PLINK)),]
# } else {
# df <- df
# }
# ## remove any outliers Post QC
# if (PC_TYPE == "PC2"){
#   df <- df[(!grepl ("27_131", df$IID_PLINK)),]
# } else if (PC_TYPE == "PC4"){
#   df <- df[(!grepl ("22_12", df$IID_PLINK)),]
# } else {
#   df <- df
# }
sp <- ggscatter(df, x = VARX, y = VARY, size = 0.2,
...
## R
# sp + stat_cor(method = "pearson", label.x = 0.00, label.y = 0.1)
## R-sq

sp <- sp + stat_cor(
  aes(label = paste(..r.label.., ..p.label.., sep = "~`,`~")), label.y = max(df[VARY]) 
)
BP[[i]] <- sp


# ggsave(paste0(PC_TYPE,"_PLINK_vs_GENESIS.jpeg"), plot = sp, device = NULL, scale = 1, width = 3, height = 3, dpi = 300, limitsize = TRUE)
}

library(gridExtra)
P <- do.call("grid.arrange", c(BP, ncol=2))
setwd("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/06-Aquilla_202101-a/01-Aquilla-preQC/03-PLINK-QC-files4/GENESIS")
ggsave(paste0("PLINK_vs_GENESIS_PRIOR_QC_after_removing_the_outlier.jpeg"), plot = P, device = NULL, scale = 1, width = 6, height = 6, dpi = 300, limitsize = TRUE)
# ggsave(paste0("PLINK_vs_GENESIS_PRIOR_QC.jpeg"), plot = P, device = NULL, scale = 1, width = 6, height = 6, dpi = 300, limitsize = TRUE)
ggsave(paste0("PLINK_vs_GENESIS_POST_QC_after_removing_the_outlier.jpeg"), plot = P, device = NULL, scale = 1, width = 6, height = 6, dpi = 300, limitsize = TRUE)
# ggsave(paste0("PLINK_vs_GENESIS_POST_QC.jpeg"), plot = P, device = NULL, scale = 1, width = 6, height = 6, dpi = 300, limitsize = TRUE)
...
jpeg("ggparcord_FASe_3982_PC1_PC2_ggplot.jpeg", width = 600, height = 700, units = "px", pointsize = 16, bg = "white")
ggparcoord(pc.df, columns=1:10, groupColumn="Population", scale="uniminmax") +
  xlab("PC") + ylab("")
dev.off()
...
(gds.fn <- SNPRelate::snpgdsOpen(gds))
(f <- seqOpen(gds.fn))
annot <- annot[match(seqGetData(gds, "sample.id"), annot$IID),]
