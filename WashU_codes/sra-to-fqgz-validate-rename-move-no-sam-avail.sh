...
IFS=$'\n'
RGLINES=($(sed 's/^ *//g' ${SRR}.rgs | cut -d\  -f2))
args=(tee)
...
#  args+=(\>\(grep -A3 --no-group-separator \"\\.${RG[1]#ID:}/\" \| gzip \> ${SRR}.${RG[1]#ID:}.fastq-dump.split.defline.z.tee.fq.gz\))
  args+=(\>\(grep -A3 --no-group-separator \"\\.${RGLINE}/\" \| gzip \> ${SRR}.${RGLINE}.fastq-dump.split.defline.z.tee.fq.gz\))
done
args+=(\>/dev/null)
...
if [ ${exitcode} -eq 0 ]; then
  ...
  for RGLINE in ${RGLINES[@]}; do
    ...
    echo "@RG\tID:${NEW_RGID}\tPL:ILLUMINA\tLB:${SRR}\tDS:${FULLSM}\tSM:${SM}" > "ready/${FULLSM}/${FULLSM}.${NEW_RGID}.rgfile"
  done
fi
unset IFS
