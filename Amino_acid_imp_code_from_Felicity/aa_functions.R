...
(
  ...
  pos.aa.types = sapply(strsplit(ids.char,split='_'),function(x) return(x[5]))
  ...
  if( (max(row.idx) - min(row.idx)) != (length(ids)-1) ) {
    cat("may have problems getting the correct data! ",i,"\n")
  }
  ...
  #cat(pos, "colsums (aa analysis)",i,mean(colsums),"\n")
  if( mean(colsums) > 2.1 | mean(colsums) < 1.9 ) {
    #cat("check colsums (aa analysis) ",i,mean(colsums),"\n")
    cat("WARN check colsums (aa analysis)",i,mean(colsums),"\n")
    name= paste("tmp",pos,"tmp.txt",sep=".")
    write.table(colsums, name, sep="\t")
  }
  if( sum(allelefreqs) > 1.1 | sum(allelefreqs) < 0.9 ) {
    cat("WARN","check rowsums (aa analysis)",sum(allelefreqs),i,"\n")
  }
  ...
  ids2 = sapply(strsplit(ids2.char,split='_'),function(x) return(x[5]))
...
(
 ...
 fam = fam
  ...
  if( length(bim.pos.ids) == 1 ) {
    ...
    cat("Multiallelic SNP. Not yet implemented\n")
...
(
 ...
 cond.data = cond.data
  ...
  if( mean(colsums) > 2.1 | mean(colsums) < 1.9 ) {
    cat("WARN","check colsums (aa analysis) ",i,"\n")
  }
  if( sum(allelefreqs) > 1.1 | sum(allelefreqs) < 0.9 ) {
    cat("WARN","check rowsums (aa analysis) ",i,"\n")
  }
