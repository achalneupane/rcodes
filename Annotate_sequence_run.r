...
UQCCG.data<-"/media/ga-apps/UQCCG" # path to project data 
project<-"FMHT"
affection.status<-c( "X1000752W.UnAff" "X1000753W.Aff"   "X1000767W.Aff"   "X1000768W.UnAff" "X1000789J.Aff"   "X1001009J.UnAff" "X1001030J.Aff"   "X1002416W.Aff")
##############################################

######################################################### Set up the basics for each run #####
UQCCG.data<-"/media/ga-apps/UQCCG-Analysis" # path to project data 
project<-"AOGC_HBM_WTCC2" # this is the project directory
project.name<-"AOGC_HBM" # this is the prefix of the snp and DINDEL files

annotate.dir<-"/media/ga-apps/UQCCG-Analysis/AOGC_HBM_WTCC2/Annotate"
snp.dir<-"/media/ga-apps/UQCCG-Analysis/AOGC_HBM_WTCC2"

skip.annovar.run<-FALSE # if FALSE it just redoes the files annovar summarization
update.annovar.annotations<-TRUE # set TRUE if want to force update . If annovar not run then FALSE will still case annovar to run (if skip.annovar.run=FALSE)
GATK.SB<-FALSE
genome.build<-"hg18"
dbSNP.build<-"130"
vcf.type="plink_assoc" #vcf.type="plink_assoc"
## vcf.type="mach_assoc"

bam.extension<-""
combined.extension<-""
snp.extension<-".assoc"  # "_snps.raw.vcf"
indel.extension<-""   # "_DINDEL.raw.vcf"  
small.extension<-""  # "_All_VARIANTS.raw.vcf"
variant.types<-c("snp") ##MUST have a extension type for each indel defined
names(variant.types)<-c("plink.assoc") ### define data type for when reading below
use.variants<-c("snp")
########################################


######################################################### Set up the basics for each run #####
UQCCG.data<-"/media/scratch2" # path to project data 
project<-"AOGC-NGS" # this is the project directory
project.name<-project # this is the prefix of the snp and DINDEL files

skip.annovar.run<-FALSE
GATK.SB<-TRUE
genome.build<-"hg19"
dbSNP.build<-"131"
vcf.type="v4"
...
combined.extension<-""
snp.extension<-"_snps.raw.vcf"  # "_snps.raw.vcf"
indel.extension<-"_DINDEL.raw.vcf"   # "_DINDEL.raw.vcf"  
...
######################################################### Set up the basics for each run #####
UQCCG.data<-"/media/ga-apps/UQCCG/Projects" # path to project data 
project<-"FMHT" # this is the project directory
project.name<-project # this is the prefix of the snp and DINDEL files

annotate.dir<-"/media/ga-apps/UQCCG/Projects/FMHT/Annotate"

skip.annovar.run<-FALSE  # if FALSE it just redoes the files annovar summarization
update.annovar.annotations<-FALSE # set TRUE if want to force update . If annovar not run then FALSE will still case annovar to run (if skip.annovar.run=FALSE)
GATK.SB<-TRUE
genome.build<-"hg19"
dbSNP.build<-"131"
vcf.type="v4"
...
######################################################### Set up the basics for each run #####
UQCCG.data<-"/media/ga-apps/UQCCG/Projects" # path to project data 
project<-"RNSH" # this is the project directory
project.name<-project # this is the prefix of the snp and DINDEL files

annotate.dir<-"/media/ga-apps/UQCCG/Projects/RNSH/Annotate"
...
######################################################### Set up the basics for each run #####
UQCCG.data<-"/media/ga-apps/UQCCG/Projects" # path to project data 
project<-"SKDP-FAM-38" # this is the project directory
project.name<-project # this is the prefix of the snp and DINDEL files


annotate.dir<-"/media/ga-apps/UQCCG/Projects/SKDP-FAM-38/Annotate"
...
project<-"2011-12-19_SN989_0126_SKDP" # this is the project directory
project.name<-"2011-12-19_SN989_0126_SKDP" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2011-12-19_SN989_0126_SKDP/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2011-12-01_SKDP" # this is the project directory
project.name<-"2011-12-01_SKDP" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2011-12-01_SKDP/Annotate"

skip.annovar.run<-FALSE # if FALSE it just redoes the files annovar summarization IF not already run
update.annovar.annotations<-FALSE # set TRUE if want to force update of ALL annotations. If annovar not run then FALSE will still cause annovar to run (if skip.annovar.run=FALSE)
GATK.SB<-FALSE
genome.build<-"hg19"
dbSNP.build<-"131"
vcf.type="v4"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2011-12-03_Mouse" # this is the project directory
project.name<-"All_Mouse" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2011-12-03_Mouse/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2012-02-20_BWA_Mouse" # this is the project directory
project.name<-"All_BWA_Mouse" # this is the prefix of the SNP and DINDEL files AND the project output NAME


annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-02-20_BWA_Mouse/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2012-03-01_NewNova_Mouse" # this is the project directory
project.name<-"All_Nova_Mouse_SNPs" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-03-01_NewNova_Mouse/Annotate"
...
######################################################### Set up the basics for each run #####
UQCCG.data<-"/media/ga-apps/UQCCG/Projects/AOGC-NGS" # path to project data 
project<-"AOGC-NGS_ALL" # this is the project directory
project.name<-project # this is the prefix of the snp and DINDEL files/media/ga-apps/UQCCG/Projects/AOGC-NGS/Annotate

annotate.dir<-"/media/ga-apps/UQCCG/Projects/AOGC-NGS/Annotate"
...
######################################################### Set up the basics for each run #####
UQCCG.data<-"/media/ga-apps/UQCCG/Projects" # path to project data 
project<-"SKDP-FAM-9" # this is the project directory
project.name<-project # this is the prefix of the snp and DINDEL files

annotate.dir<-"/media/ga-apps/UQCCG/Projects/SKDP-FAM-9/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2012-05-25_SKDP_Calling_All_GAII" # this is the project directory
project.name<-"SKDP_Calling_All_GAII" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-05-25_SKDP_Calling_All_GAII/Annotate"

skip.annovar.run<-FALSE # if FALSE it just redoes the files annovar summarization IF not already run
update.annovar.annotations<-TRUE # set TRUE if want to force update of ALL annotations. If annovar not run then FALSE will still cause annovar to run (if skip.annovar.run=FALSE)
GATK.SB<-FALSE
genome.build<-"hg19"
dbSNP.build<-"131"
vcf.type="v4"
...
######################################################### Set up the basics for each run #####
UQCCG.data<-"/media/ga-apps/UQCCG/Projects" # path to project data 
project<-"SKDP-FAM-6" # this is the project directory
project.name<-project # this is the prefix of the snp and DINDEL files

annotate.dir<-"/media/ga-apps/UQCCG/Projects/SKDP-FAM-6/Annotate"

skip.annovar.run<-FALSE  # if FALSE it just redoes the files annovar summarization
update.annovar.annotations<-FALSE # set TRUE if want to force update and RE-read VCF . If annovar not run then FALSE will still case annovar to run (if skip.annovar.run=FALSE)
GATK.SB<-TRUE
genome.build<-"hg19"
dbSNP.build<-"131"
vcf.type="v4"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2012-03-19_SKDP_2_Runs" # this is the project directory
project.name<-"75_0080" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-03-19_SKDP_2_Runs/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/Bioinform-D/Research" # path to project data 
project<-"Phil_loci" # this is the project directory
project.name<-"Phil_loci" # this is the prefix of the SNP and DINDEL files AND the project output NAME
target<-project.name
annotate.dir<-"/media/Bioinform-D/Research/immuno_anno/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/Bioinform-D/Research" # path to project data 
project<-"immuno_anno" # this is the project directory
project.name<-"Eu_Locus" # this is the prefix of the SNP and DINDEL files AND the project output NAME
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2012-04-30_QBI" # this is the project directory
project.name<-"QBI_All" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-04-30_QBI/Annotate"
...
bam.extension<-".ReCal.sort.bam"
combined.extension<-""
snp.extension<-"_VBMN_VARIANTS.raw.vcf"  # "_snps.raw.vcf"
indel.extension<-"_DINDEL_VARIANTS.raw.vcf"   # "_DINDEL.raw.vcf"  
small.extension<-"_DINDEL_VARIANTS.raw.vcf"  # "_All_VARIANTS.raw.vcf"
variant.types<-c("snp","indel","small") ##MUST have a extension type for each indel defined
names(variant.types)<-c("v4","v4","v4") ### define data type for when reading below
use.variants<-c("snp","indel")
###############################################
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2012-05-22_SKDP_75_0085" # this is the project directory
project.name<-"SKDP-75_85" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-05-22_SKDP_75_0085/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Projects" # path to project data 
project<-"SKDP-FAM-171" # this is the project directory
project.name<-"89_0144" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Projects/SKDP-FAM-171/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2012-07-19_GroupCallGAII" # this is the project directory
project.name<-"GroupCallGAII" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-07-19_GroupCallGAII/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/Bioinform-D/Research/annotate" # path to project data 
project<-"MS snps" # this is the project directory
project.name<-"MS_SNPS" # this is the prefix of the SNP and DINDEL files AND the project output NAME # OR of bim or assoc name

annotate.dir<-"/media/Bioinform-D/Research/annotate/MS snps/Annotate"
...
genome.build<-"hg19"
dbSNP.build<-"131"
vcf.type="plink_assoc" # "annovar" "v4" "v3" "plink_assoc"

bam.extension<-".ReCal.sort.bam"
combined.extension<-""
snp.extension<-".bim"  # "_snps.raw.vcf"
indel.extension<-".indelFiltered.vcf"   # "_DINDEL.raw.vcf"  
small.extension<-"_All_VARIANTS.raw.vcf"  # "_All_VARIANTS.raw.vcf"
variant.types<-c("snp","indel","small") ##MUST have a extension type for each indel defined
names(variant.types)<-c("v4","v4","v4") ### define data type for when reading below
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2012-11-12_GroupCallQIMR" # this is the project directory
project.name<-"GCJL" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-11-12_GroupCallQIMR/Annotate"
#annotate.dir<-"/media/scratch2/Annotate"

affection.status<-"sample_sheet"
the.sample.sheet<-"GroupCallQIMR_SampleSheet_GUT.csv"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Projects" # path to project data 
project<-"BTCK" # this is the project directory
project.name<-"BTCK" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Projects/BTCK/Annotate"
#annotate.dir<-"/media/scratch2/Annotate"
affection.status<-"sample_sheet"
the.sample.sheet<-"GroupCallQIMR_SampleSheet.csv"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2013-01-23_GroupCallSKDP_Run75_119_WDR60" # this is the project directory
project.name<-"2013-01-23_GroupCallSKDP_Run75_119_WDR60" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2013-01-23_GroupCallSKDP_Run75_119_WDR60/Annotate"
#annotate.dir<-"/media/scratch2/Annotate"

affection.status<-"sample_sheet"
the.sample.sheet<-"GroupCallSKDP_Run75_119_WDR60_SampleSheet_all.csv"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Projects" # path to project data 
project<-"TBX21annotation" # this is the project DIRSCTORY
project.name<-"chr17_snps" # this is the PREFIX of the SNP and DINDEL files AND the project output NAME # OR of bim or assoc name

annotate.dir<-"/media/ga-apps/UQCCG/Projects/TBX21annotation/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/scratch2/" # path to project data 
project<-"GCC" # this is the project directory
project.name<-"GCA_WTCCc_final" # this is the prefix of the SNP and DINDEL files AND the project output NAME # OR of bim or assoc name

annotate.dir<-"/media/scratch2/GCC/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Projects" # path to project data 
project<-"TGCM-AML" # this is the project directory
project.name<-"TGCM_AML" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Projects/TGCM-AML/Annotate"
#annotate.dir<-"/media/scratch2/Annotate"
...
UQCCG.data<-"/media/scratch2" # path to project data 
project<-"AOGC-NGS" # this is the project directory
project.name<-"AOGC-NGS" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/scratch2/AOGC-NGS/Annotate"
...
genome.build<-"hg19"
dbSNP.build<-"131"
vcf.type="annovar" # "annovar" "v4" "v3" "plink_assoc"

bam.extension<-".ReCal.sort.bam"
combined.extension<-""
snp.extension<-"_indels.txt"  # "_snps.raw.vcf"
indel.extension<-".indelFiltered.vcf"   # "_DINDEL.raw.vcf"  
small.extension<-"_All_VARIANTS.raw.vcf"  # "_All_VARIANTS.raw.vcf"
variant.types<-c("snp","indel","small") ##MUST have a extension type for each indel defined   variant.types<-c("snp","indel","small")
names(variant.types)<-c("v4","v4","v4") ### define data type for when reading below
use.variants<-c("snp")
###############################################
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2013-02-21_GroupCallSKDP_Run75_119_WDR60_ContaminatedPlates" # this is the project directory
project.name<-"2013-02-21_GroupCallSKDP_Run75_119_WDR60_ContaminatedPlates" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2013-02-21_GroupCallSKDP_Run75_119_WDR60_ContaminatedPlates/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2013-02-27_AML_with_AOGCControl" # this is the project directory
project.name<-"2013-02-27_AML_with_AOGCControl" # this is the prefix of the SNP and DINDEL files AND the project output NAME
...
skip.annovar.run<-FALSE # if FALSE it just redoes the files annovar summarization IF not already run
update.annovar.annotations<-FALSE# set TRUE will re-read VCF file and force annovar to always run
GATK.SB<-TRUE
genome.build<-"hg19"
dbSNP.build<-"131"
vcf.type="v4" # "annovar" "v4" "v3" "plink_assoc"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES/media/scratch2/AOGC-NGS/
UQCCG.data<-"/media/scratch2" # path to project data
project<-"AOGC-NGS" # this is the project DIRECTORY
project.name<-"AOGC-Genotyping" # this is the prefix of the SNP and DINDEL files AND the project output NAME
...
max.reads<-7000
remove.extensions<-c(".GQ",".DP",".PL") # remove.extensions<-{}
remove.cols<-{} ## {} for none OR  c(x,y)
###############################################

###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2012-08-27_Run89_0150" # this is the project directory
project.name<-"89_150" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-08-27_Run89_0150/Annotate"
analysis.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-08-27_Run89_0150/Analysis"

affection.status<-"sample_sheet"
the.sample.sheet<-"Run89_0150-SampleSheet.csv"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2013-04-24_Run89_170_PrenatalSKDP_PCC" # this is the project directory
project.name<-"2013-04-24_Run89_170_PrenatalSKDP_PCC" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2013-04-24_Run89_170_PrenatalSKDP_PCC/Annotate"
analysis.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2013-04-24_Run89_170_PrenatalSKDP_PCC/Analysis"

affection.status<-"sample_sheet"
the.sample.sheet<-"SKDP_DEMULTI_SampleSheet.csv"
...
annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-05-22_SKDP_75_0085/Annotate"
analysis.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-05-22_SKDP_75_0085/Analysis"

affection.status<-"sample_sheet"
the.sample.sheet<-"SKDP-75_85_SampleSheet.csv"
...
annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-03-19_SKDP_2_Runs/Annotate"
analysis.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2012-03-19_SKDP_2_Runs/Analysis"


affection.status<-"sample_sheet"
the.sample.sheet<-"SKDP-2_Runs_SampleSheet.csv"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/scratch2" # path to project data 
project<-"cervical cancer final" # this is the project directory
project.name<-"all_samples_qc" # this is the prefix of the SNP and DINDEL files AND the project output NAME # OR of bim or assoc name

annotate.dir<-"/media/scratch2/cervical cancer final/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2013-05-20_GroupCallingSKDPwithBWAandRun_89_170" # this is the project directory
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes" # path to project data 
project<-"2013-06-25_GroupCallHBM_AOGC" # this is the project directory
project.name<-"2013-06-25_GroupCallHBM_AOGC" # this is the prefix of the SNP and DINDEL files AND the project output NAME

annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2013-06-25_GroupCallHBM_AOGC/Annotate"
analysis.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2013-06-25_GroupCallHBM_AOGC/Analysis"

affection.status<-"sample_sheet"
the.sample.sheet<-"2013-06-25_GroupCallHBM_AOGC.csv"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/scratch2/AOGC-NGS/GFOS" # path to project data 
project<-"uk10k.1kg.genome.legend" # this is the project directory
project.name<-"uk10k.1kg" # this is the prefix of the SNP and DINDEL files AND the project output NAME # OR of bim or assoc name

annotate.dir<-"/media/scratch2/AOGC-NGS/GFOS/Annotate2"
...
genome.build<-"hg19"
dbSNP.build<-"137"
vcf.type="legend" # "annovar" "v4" "v3" "plink_assoc"

bam.extension<-".ReCal.sort.bam"
combined.extension<-""
snp.extension<-".legend"  # "_snps.raw.vcf"
indel.extension<-".indelFiltered.vcf"   # "_DINDEL.raw.vcf"  
small.extension<-"_All_VARIANTS.raw.vcf"  # "_All_VARIANTS.raw.vcf"
variant.types<-c("snp","indel","small") ##MUST have a extension type for each indel defined
names(variant.types)<-c("v4","v4","v4") ### define data type for when reading below
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG-Analysis" # path to project data 
project<-"AOGC_exome_chip" # this is the project directory
project.name<-"zCall_AOGC.with.evoker_corrected_clean_FINAL" # this is the prefix of the SNP and DINDEL files AND the project output NAME # OR of bim or assoc name

annotate.dir<-"/media/ga-apps/UQCCG-Analysis/AOGC_exome_chip/Annotate"

skip.annovar.run<-FALSE # if FALSE it just redoes the files annovar summarization IF not already run
update.annovar.annotations<-TRUE # set TRUE will re-read VCF file and force annovar to always run
GATK.SB<-TRUE
genome.build<-"hg19" ## bim file on this build
dbSNP.build<-"131"
vcf.type="bim" # "annovar" "v4" "v3" "plink_assoc" "bim"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/Bioinform-D/Research" # path to project data 
project<-"GWAS_Figures" # this is the project directory
project.name<-"ALL_eth_650Y_forward_hg19" # this is the prefix of the SNP and DINDEL files AND the project output NAME # OR of bim or assoc name

annotate.dir<-"/media/Bioinform-D/Research/GWAS_Figures/Annotate"
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
UQCCG.data<-"/media/ga-apps/UQCCG-Analysis/AOGC_exome_chip" # path to project data 
project<-"PCA_calc" # this is the project directory
project.name<-"AOGC_merge_common" # this is the prefix of the SNP and DINDEL files AND the project output NAME # OR of bim or assoc name
...
UQCCG.data<-"/media/ga-apps/UQCCG-Analysis/AOGC_exome_chip" # path to project data 
project<-"PCA_calc" # this is the project directory
project.name<-"HBM_final_to_impute" # this is the prefix of the SNP and DINDEL files AND the project output NAME # OR of bim or assoc name
...
annotate.dir<-"/media/ga-apps/UQCCG/Data/Sequence_Genotypes/2013-10-27_AML_with_AOGCControl_NoFailedLane/Annotate"
...
force.functional.update<-FALSE # will re-read functional data normally set to TRUE.  If have functional.RData file already set to FALSE (will ALWAYS chech is pholyPhen run is TRUE)
force.GERP.update<-FALSE  # will  get scores normally set to TRUE # force.GERP.update<-FALSE
update.annotations.with.vep<-FALSE ##undate gene names and geneanno.table with filter.table.poly... wanted muts traken care of below
skip.gene.list.matches<-FALSE ### usually false match against lists of gene names
...
if(inherits(xx, "try-error")){polyphen.dir<-project.dir}
...
regionanno.DB<-c("jaxQtlAsIs","gwasCatalog","omimGene","segdup","tfbsConsSites","dgv","mirna")  ##use colsWanted -all so must have colun labels defined c("jaxQtlAsIs","gerp++elem","Gerp","gwasCatalog","omimGene","segdup","tfbsConsSites","dgv","mirna") 
names(regionanno.DB)<-c("jaxQtlAsIs","gwasCatalog","omimGene","segdup","tfbsConsSites","dgv","mirna")  ##use colsWanted -all so must have c("jaxQtlAsIs","gerpelem","Gerp","gwasCatalog","omimGene","segdup","tfbsConsSites","dgv","mirna")
...
filter.DB<-c("snp137","snp132","1000g2012apr_all","1000g2012apr_asn")  # returns 2  extra columns (DB,score) c("snp132","1000g2011may_all")
names(filter.DB)<-c("ID","snp132","1000genome","1000genome_asian") # these will be the names in the summary table
...
generic.filter.DB<-c("hg19_esp6500si_all.txt","hg19_NHBLI_6500_ALL_maf.txt","hg19_esp6500_ea.txt","hg19_esp6500_aa.txt","hg19_snp137_pubmed_maf.txt","hg19_snp137_omim_maf.txt","hg19_snp137_clinical_maf.txt","hg19_snp137_maf.txt","hg19_1000_genomes_feb_maf.txt","hg19_snp135_maf.txt","hg19_snp135_pathalogical_maf.txt","hg19_snp135_clinical_maf.txt","hg19_NHBLI_5400_ALL_maf.txt","hg19_NHBLI_5400_EA_maf.txt","hg19_NHBLI_5400_AA_maf.txt","hg19_NHBLI_5400_ALL_QCFail_maf.txt","hg19_NHBLI_5400_EA_QCFail_maf.txt","hg19_NHBLI_5400_AA_QCFail_maf.txt","hg19_cg69.txt","hg19_EUR_ASN_AFR_DINDEL_maf.txt","AOGC-NGS_ALL_maf.txt","AOGC-NGS_ALL_QCFail_maf.txt","AOGC-NGS_ALL_OLD_maf.txt","AOGC-NGS_ALL_OLD_QCFail_maf.txt","hg19_Chinese_maf.txt")    # returns 2  extra columns (DB,score)

names(generic.filter.DB)<-c("NHBLI_6500_ANNOVAR_ALL","NHBLI_6500_ALL","NHBLI_6500_EA","NHBLI_6500_AA","snp137_pubmed","snp137_omim","snp137_clinical","snp137","1000genome_mine","snp135","snp135_pathalogical","snp135_clinical","NHLBI_5400_ALL","NHLBI_5400_EUR","NHLBI_5400_AFR","NHLBI_5400_ALL_QCFail","NHLBI_5400_EUR_QCFail","NHLBI_5400_AFR_QCFail","CG69","EUR_ASN_AFR_INDEL","AOGC-NGS_ALL","AOGC-NGS_ALL_QCFail","AOGC-NGS_ALL_OLD","AOGC-NGS_ALL_OLD_QCFail","Chinese")


#### thies ones don't have a score column
function.filter.DB<-c("ljb_pp2","avsift","ljb_mt","ljb_phylop","cosmic61") # returns 2  extra columns (DB,score) c("gerp++gt2","ljb_gerp++")
names(function.filter.DB)<-c("pholyphen","sift","mut.taster","phylo","cosmic")                     #                  c("GERP","ljb_gerp")
...
anno.DB.location.core<-"/media/Bioinform-D/Research/annovar/mousedb"
anno.DB.location<-paste(anno.DB.location.core,genome.build,sep="/")
...
if(inherits(xx, "try-error")){system(paste("mkdir",paste("'",annotate.dir,"'",sep=""),sep=" "));setwd(annotate.dir  )}
...
## gene.desc<-getBM( filters = "embl", values=genes, mart = mart)  gene.desc<-getBM(attributes = gene.ann.wanted, filters = "ensembl_gene_id", values=genes, mart = mart)
...
colnames(pot.genes)<-c("Potassium")
   ...
   save(list=c("ALS","omim","ingenuity.bone.genes","Dequeant.cycling","mouse.defect","sewell.cycling","skeletome","pot.genes","HypoMag","ProtonPump"),file=paste(gsub(".txt$","",target),".GeneLists.RData",sep=""))
  #########################################################

if(project=="TGCM-AML" |  project=="2013-02-27_AML_with_AOGCControl"){
...
## the.gen.labels<-c("0/0","0/1","1/1","NA")
## x<-indels[1:5,the.gen]
## counts<-apply(indels[1:5000,the.gen],1,function(x) {tmp<-tapply(x,x,length)
##                                          tmp[the.gen.labels]})
## library(foreach)  ## could not get this to work well
## library(doMC)
## registerDoMC(cores=7)
## do.counts<-function(x) {tmp<-tapply(t(x[1,]),t(x[1,]),length) ## note that foreach sents out a dataframe see
##                         tmp[c("0/0","0/1","1/1","NA")]}
## counts<-foreach(x=iter(indels[,the.gen], by='row'), .combine=cbind,.multicombine=TRUE,.inorder=TRUE) %dopar% do.counts(x)
## counts<-t(counts)
##colnames(counts)<-the.gen.labels
 indels[indels[,"start"]==94037647,]     

the.gen<-expand.labels.to.samples(c("GT"),all.sample.labels)
counts<-genotype.summary(indels[,the.gen])
colnames(counts)<-c("MAF","ALT.Alleles","REF.Alleles","TOTAL.Alleles","MISSING.Alleles","1/1","0/1","GENO")
...
the.gen<-expand.labels.to.samples(c("GT"),all.sample.labels)
the.AN<-expand.labels.to.samples(c("AN"),all.sample.labels)

summary<-cbind(indels[,core.ann],geneanno.table,Gene.Names,wanted.muts,wanted.muts.coding,good.qual,wanted.muts.fil,ok.in.group,ok.balance.in.group,maf.lt.all,summary.geno.group,summary.depths.group,indels[,the.gen]


gen.summary<-cbind(indels[,core.ann],geneanno.table,Gene.Names,counts,indels[,the.gen])
write.table(gen.summary,file=paste(target,"genetic.summary.txt",sep="."),col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)
write.table(gen.summary[maf.lt.all[,dim(maf.lt.all)[2]],],file=paste(target,"genetic.summary.maf.filtered.txt",sep="."),col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)


gen.summary<-cbind(indels[,core.ann],geneanno.table,Gene.Names,counts)
write.table(gen.summary,file=paste(target,"genetic.summary.short.txt",sep="."),col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)
write.table(gen.summary[maf.lt.all[,dim(maf.lt.all)[2]],],file=paste(target,"genectic.summary.short.maf.filtered.txt",sep="."),col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)
...
interesting.mutations<-c("UTR3","UTR5","UTR5;UTR3","ncRNA_splicing","ncRNA_UTR3","ncRNA_UTR5","ncRNA_UTR5;ncRNA_UTR3","frameshift substitution","nonframeshift substitution","nonframeshift deletion","nonframeshift insertion","frameshift deletion","frameshift insertion","nonsynonymous SNV","stopgain SNV","stoploss SNV","splicing","ncRNA_exonic")


interesting.coding.mutations<-c("frameshift substitution","nonframeshift deletion","nonframeshift insertion","frameshift deletion","frameshift insertion","nonsynonymous SNV","stopgain SNV","stoploss SNV","splicing")
...
interesting.coding.mutations




wanted.muts.coding<-rep(FALSE,times=dim(geneanno.table)[1])
for(i in 1:length(geneanno.DB)){
  the.test<-test.wanted.mutation(geneanno.table[,paste(geneanno.DB[i],"location",sep="::")],interesting.coding.mutations,delimit.by=";")
  the.test[is.na(the.test)]<-FALSE # if no annotion set to FALSE for wanted   sum(the.test[is.na(the.test)])
  wanted.muts.coding<- wanted.muts.coding | the.test
}
sum(wanted.muts.coding)

## wanted.muts<-rep(FALSE,times=dim(geneanno.table)[1])
## for(i in 1:length(geneanno.DB)){
##   the.test<-test.wanted.mutation(geneanno.table[,paste(geneanno.DB[i],"location",sep="::")],interesting.mutations,delimit.by=";")
##   the.test[is.na(the.test)]<-FALSE # if no annotion set to FALSE for wanted   sum(the.test[is.na(the.test)])
##   wanted.muts<- wanted.muts | the.test
## }

############## noncoding double filtered for biotype ###
interesting.NONcoding.mutations<-interesting.mutations[!(interesting.mutations %in% interesting.coding.mutations)]
wanted.muts.NONcoding<-rep(FALSE,times=dim(geneanno.table)[1])
interesting.NONcoding.mutations
for(i in 1:length(geneanno.DB)){
  the.test<-test.wanted.mutation(geneanno.table[,paste(geneanno.DB[i],"location",sep="::")],interesting.NONcoding.mutations,delimit.by=";")
  the.test[is.na(the.test)]<-FALSE # if no annotion set to FALSE for wanted   sum(the.test[is.na(the.test)])
  wanted.muts.NONcoding<- wanted.muts.NONcoding | the.test
}
...
gene.desc.table.labels.wanted<-c("description","gene_biotype") # gene.desc.table.labels.wanted<-c("description")
gene.desc.table.labels.other<-colnames(gene.desc.table)[!( colnames(gene.desc.table) %in% gene.desc.table.labels.wanted )]

Gerp.score.ori<-regionanno.table[, "Gerp::chrom, chromStart, chromEnd, score"]
Gerp.score<-strsplit(Gerp.score.ori,split=",")

tapply(unlist(lapply(Gerp.score,length)),unlist(lapply(Gerp.score,length)),length)


true.Gerp<-{}
true.score<-{}
for(i in 1:dim(indels)[1]){
the.gerp<-Gerp.score[[i]]
the.posn<-as.numeric(indels[i,"start"])
the.score<-as.numeric(unlist(strsplit(the.gerp[1],split=":"))[4])
the.location<-1
if (length(the.gerp)==2){
 the.score2<-as.numeric(unlist(strsplit(the.gerp[2],split=":"))[4])
 a.start<-as.numeric(unlist(strsplit(the.gerp[2],split=":"))[2])
 a.end<-as.numeric(unlist(strsplit(the.gerp[2],split=":"))[3])
 if((the.posn> a.start) & (the.posn< a.start)){the.score<-the.score2; the.location<-2}
}
true.Gerp<-c(true.Gerp,Gerp.score[[i]][the.location])
true.score<-c(true.score,the.score)
}
regionanno.table[, "Gerp::chrom, chromStart, chromEnd, score"]<-true.Gerp
Gerp.score<-true.score
Gerp.score[is.na(Gerp.score)]<-0

test<-cbind(indels[,core.ann],filter.table[,"hg19_Gerp_Annovar_1.0.txt::maf"] ,Gerp.score) 
write.table(test,file="problems.txt",col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)

other.gerp<-as.numeric(filter.table[,"hg19_Gerp_Annovar_2.0.txt::maf"])

cbind(Gerp.score,other.gerp)[1:20,]
missing<-is.na(other.gerp)
Gerp.score[!missing]<-other.gerp[!missing]


summary<-cbind(indels,geneanno.table,Gene.Names,gene.desc.table[,gene.desc.table.labels.wanted],wanted.muts.NONcoding,wanted.muts.coding,Gerp.score,gene.desc.table[,gene.desc.table.labels.other],regionanno.table,filter.table)

summary[, "gerpelem::chrom, chromStart, chromEnd, score"]

write.table(summary,file=paste(project,"analysis2.txt",sep="."),col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)

 save(list=c("target","project.name","geneanno.DB","indels","key.indels","geneanno.table","gene.desc.table","regionanno.table","filter.table","gene.names","maf.threshold.filter","the.combined.DBs","all.filter.cols.novel","all.filter.cols.maf","all.sample.labels" , "omim","ingenuity.bone.genes","Dequeant.cycling","mouse.defect","sewell.cycling","skeletome","pot.genes","interesting.mutations","HypoMag","ProtonPump"),file=paste(gsub(".txt","",target),".table.RData",sep="")) 



> + + + + + + + + [1] "Error missing annotations- removing -DANGEROUS"
[1] "chr1:33318868:33318868:-:ATTTGNGGTCAGGA:indel"                                    "chr17:78081526:78081526:-:GCAGCGN:indel"                                         
[3] "chr22:26346484:26346484:-:TGTGTGTGTATGTGTGTNTG:indel:26346484"                    "chr22:26346484:26346502:ATGTGTGTGTATGTGTGTG:ATGTGTGTGTATGTGTGTNTG:indel:26346484"
[5] "chr22:26346484:26346504:ATGTGTGTGTATGTGTGTNTG:ATGTGTGTGTG:indel:26346484"         "chrX:114425181:114425181:-:GAGGCCGCTCGCCCAACGCCCACAGCN:indel" 
