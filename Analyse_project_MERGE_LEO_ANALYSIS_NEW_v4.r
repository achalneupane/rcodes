...
pass_coding<- full.qual &  bad.coding & maf.filter   & !in.common.hit.gene   & !unannotated.hits & not.flat.genotype  &
  ok.missing & hw.controls.ok.filt & !no.genotypes.filt &  rare.in.Control.filt & !in.any.normal.filt & !in.any.normal
...
write.table(weights,file="/home/fnewell/PBSHOME/Projects/NSLM/somatic_signatures/new_weights.txt",sep="\t",row.names=F,col.names=T,quote=F)
class(weigh)
weights[,2]

plot(as.numeric(x),as.numeric(y),main="For C>T motif SNPs",xlab="Synonymous",ylab="NonSynonymous",cex.lab=1.5)
...
snap.file
snap.file<-paste(snap.file,".withGQ.filter.QCed_weights_possonian_rescue_6sd",sep="")
snap.file
...
# synonymous<-test.for.coding.type(a.indel,geneanno.DB,synonymous.variant)
# 
# synonymous.vep<-test.wanted.mutation(a.indel[,"Consequence.Embl"],synonymous.variant,delimit.by=",")  # filter.table.pholy[,"Consequence"] %in%  vep.coding
# sum(synonymous.vep)
# 
# is.synonymous<-synonymous | synonymous.vep
# is.synonymous<-synonymous 
# sum(synonymous)
# 
# 
# pass_coding<- full.qual &  is.missense & maf.filter   & !in.common.hit.gene   & !unannotated.hits & not.flat.genotype  &
#   ok.missing & hw.controls.ok.filt & !no.genotypes.filt &  rare.in.Control.filt & !in.any.normal.filt & !in.any.normal
# 
# pass_non.coding<- full.qual &  is.synonymous & maf.filter   & !in.common.hit.gene   & !unannotated.hits & not.flat.genotype  &
#   ok.missing & hw.controls.ok.filt & !no.genotypes.filt &  rare.in.Control.filt & !in.any.normal.filt & !in.any.normal
# 
# sum(pass_coding)
# sum(pass_non.coding)
...
pass_coding_snps<-a.indel[pass_coding,]
pass_non.coding_snps<-a.indel[pass_non.coding,]


colnames(a.indel)[1:50]

sort(table(a.indel[pass,"Consequence.Embl"]))

sort(table(a.indel[!pass,"wanted.muts"]))
...
meta.results.skatO<-skatOMeta(cohort.seq,skat.wts="gene.weights.subset",burden.wts ="gene.weights.subset",SNPInfo = snpinfo,aggregateBy="cluster",method = "integration")
...
to.unwind.name<-"TOP_350_LENGTH_CONTROL"
...
subset<-subset[!is.na(as.numeric(subset[ ,"p"])),]
       ...
       subset<-meta.results.burden[!is.na(as.numeric(meta.results.burden[ ,"p"])),]
