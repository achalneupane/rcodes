...
#project<-"2014-06-24_AML_RSGB_AOGC_withHaplotypeCaller" # this is the project directory
#project<-"2015-01-15_LeoPharma_Dec2014Freeze" # this is the project directory
project.name <- project
annotate.dir <-paste(UQCCG.data,project,"Annotate",sep="/")
analysis.dir<-paste(UQCCG.data,project,"Analysis",sep="/")

affection.status.file<-the.sample.sheet # sample_sheet from demutiplex , coverage_file - coverge file , no_file
# the.sample.sheet<-"/mnt/UQCCG/Sequencing/Data/Sequence_Genotypes/2014-03-19_MND_MODY_LKAF_Nimbelgen/BAM/Ch_MND_SampleSheet.csv"
 #the.sample.sheet<-"/mnt/UQCCG/Sequencing/Data/Sequence_Genotypes/Data_Freeze_ChNMD_AOGC_Tulane/MND_AffStatus.csv"
restrict.family<-c("ALL") #not required for affection.status.file="no_file"- set to "ALL" ALL or from "Project" in coverge or "SampleProject"  in samplesheet
...
biomart.gene.desc.file<-"/mnt/UQCCG/Software/annovar/humandb/Biomart.gene.desc.RData"
ensg.to.HGNC.map.file<-"/mnt/UQCCG/Sequencing/Data/Genomes/hg19/ENSG_to_HGNC.txt"

filter.cols.novel<-c("NHBLI_6500_ANNOVAR_ALL","NHBLI_6500_ALL","NHLBI_5400_ALL","NHLBI_5400_EUR","NHLBI_5400_AFR","1000genome","1000genome_asian","1000genome_mine","snp137","snp137_clinical","snp135","CG69","EUR_ASN_AFR_INDEL","AOGC-NGS_ALL","AOGC-NGS_ALL_OLD","Chinese") ##
filter.cols.maf<-c("NHBLI_6500_ANNOVAR_ALL","NHBLI_6500_ALL","NHBLI_6500_EA","NHBLI_6500_AA","NHLBI_5400_ALL","1000genome","snp137","snp141","PopFreqMax")
...
if(!exists("annotate.dir")){annotate.dir<-"/dmf/uqdi/Core_Services/Bioinform-D/Research/annotate"}
...
all.possible.samples<-gsub(".GT$","",colnames(indels)[grep(".GT$",colnames(indels))],perl=TRUE)
length(all.possible.samples)


if(sum(grepl(".VR$",colnames(indels)))>1  & sum(grepl(".RR$",colnames(indels)))>1 ){
colnames(indels)[grep(all.possible.samples[1],colnames(indels))]
iv<-1
for(iv in 1:length(all.possible.samples)){
temp<-paste(  indels[,paste(all.possible.samples[iv],".RR",sep="")],indels[,paste(all.possible.samples[iv],".VR",sep="")],sep=",")
if(iv==1){all.temp<-temp}else{all.temp<-cbind(all.temp,temp)}
}
colnames(all.temp)<-paste(all.possible.samples,".AD",sep="")
colnames(all.temp)
indels<-cbind(indels,all.temp)
}
##########################
  ## indel.ori<-indels
  ## load("indels.RData")
...
wanted.muts.coding <- wanted.muts.coding | wanted.muts.coding.vep
 ...
 #prefilter<-maf.lt.all[,dim(maf.lt.all)[2]]
  ...
  sum(wanted.muts)
  prefilter<-  maf.lt.all[,dim(maf.lt.all)[2]] #### what will be printed out
...
if(Prefilter.all.data){
 # prefilter<-wanted.muts & maf.lt.all[,dim(maf.lt.all)[2]] ### wanted and Rare
  prefilter<- maf.lt.all[,dim(maf.lt.all)[2]] ### wanted and Rare
  sum(prefilter)
 ## current.objects<-ls()
 ##  io<-11
...
ens.to.hgnc<-read.delim(ensg.to.HGNC.map.file,header=T,sep="\t",fill=TRUE,stringsAsFactors=FALSE,na.strings="",quote="\"")
ens.to.hgnc[1:5,]
...
print(file.out.name)
write.table(summary.reduced,file=paste(file.out.name,"ALL_GENOTYPES_analysis-maf-filtered.txt",sep="."),col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)
