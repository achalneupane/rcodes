...
##  affection.status.file<-"sample_sheet"
...
##  the.sample.sheet<-"/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2014-06-27_All_AOGC_HC/BAM/aogc_QC_ok_sample.sheet.csv"

 
##  restrict.family<-c("AOGC")
...
##  affection.status.file<-"coverage.file" # sample_sheet from demutiplex , coverage_file - coverge file 
## # the.sample.sheet<-"/mnt/UQCCG/Sequencing/Data/Sequence_Genotypes/2014-03-19_MND_MODY_LKAF_Nimbelgen/BAM/Ch_MND_SampleSheet.csv"
...
##   restrict.family<-c("NSLM") # from "Project" in coverge or "SampleProject"  in samplesheet



## UQCCG.data<-"/home/mmarshall/PBSHOME/2014-08-21_ChMND/"
## #UQCCG.data<-"/media/UQCCG/Sequencing/Data/Sequence_Genotypes"
## #project<-"2014-06-24_AML_RSGB_AOGC_withHaplotypeCaller" # this is the project directory
## project<-"2014-08-21_MND_AOGC_Tulane" # this is the project directory
## project.name <- project
## annotate.dir <-paste(UQCCG.data,project,"Annotate",sep="/")
## analysis.dir<-paste(UQCCG.data,project,"Analysis",sep="/")
...
#project<-"2014-06-24_AML_RSGB_AOGC_withHaplotypeCaller" # this is the project directory
project<-"2014-10-09_AML_CompleteGenomics_HC" # this is the project directory
project.name <- project
annotate.dir <-paste(UQCCG.data,project,"Annotate",sep="/")
analysis.dir<-paste(UQCCG.data,project,"Analysis",sep="/")
...
affection.status.file<-"no_file" # sample_sheet from demutiplex , coverage_file - coverge file , no_file
# the.sample.sheet<-"/mnt/UQCCG/Sequencing/Data/Sequence_Genotypes/2014-03-19_MND_MODY_LKAF_Nimbelgen/BAM/Ch_MND_SampleSheet.csv"
 the.sample.sheet<-"/mnt/UQCCG/Sequencing/Data/Sequence_Genotypes/Data_Freeze_ChNMD_AOGC_Tulane/MND_AffStatus.csv"
restrict.family<-c("ALL") #nor required for affection.status.file="no_file"- set to "ALL" ALL or from "Project" in coverge or "SampleProject"  in samplesheet
...
#include.control.in.other<-TRUE # include.control.in.other<-FALSE
make.nonDefined.samples.as.OTHER<-TRUE
include.homozygotes<-FALSE ## used for cancer
###############################################
...
load("/media/UQCCG/Software/annovar/humandb/Biomart.gene.desc.RData")
  dups<-duplicated(gene.desc[,1])
  gene.desc<-gene.desc[!dups,]
####################### match genes to another column #########################################
...
gene.desc.table[1:5,]
colnames(gene.desc.table)[colnames(gene.desc.table)=="gene_biotype"]<-"Gene.Biotype"
colnames(gene.desc.table)[colnames(gene.desc.table)=="description"]<-"Description"
...
wanted.muts.coding <- wanted.muts.coding | wanted.muts.coding.vep
 ...
 prefilter<- maf.lt.all[,dim(maf.lt.all)[2]]
...
#####################################################
 ...
 ########################
no.gene<-is.na(Gene.Names) | Gene.Names=="NA"

if(sum(no.gene>0)){
# sum(no.gene)
all.genes<-sort(table(Gene.Names),decreasing=TRUE)
#all.genes[1:50]

ens.to.hgnc<-read.delim("/mnt/UQCCG/Sequencing/Data/Genomes/hg19/ENSG_to_HGNC.txt",header=T,sep="\t",fill=TRUE,stringsAsFactors=FALSE,na.strings="",quote="\"")
ens.to.hgnc[1:5,]
genes<-unique(filter.table.pholy[no.gene,"Gene"]) # will become final "Gene.Embl" column in indels
...
posns<-match(filter.table.pholy[no.gene,"Gene"],ens.to.hgnc[,"Ensembl.Gene.ID"])
missing<-is.na(posns)
Gene.Names[no.gene]<-ens.to.hgnc[posns,"HGNC.symbol"]
#Gene.Names[no.gene]
 
#ens.to.hgnc[posns,"HGNC.symbol"][1:30]
#gene.names$"refGene::gene"[no.gene][1:10]
} # some were ens to hgne genes
########### try just ENSGENE

no.gene<-is.na(Gene.Names) | Gene.Names=="NA"
 sum(no.gene)
if(sum(no.gene>0)){
Gene.Names[no.gene]<-filter.table.pholy[no.gene,"Gene"]
a.dash<-Gene.Names=="-"
sum(a.dash)
Gene.Names[a.dash]
Gene.Names[a.dash]<-filter.table.pholy[a.dash,"Feature"]
 } # use features

## Unknown intergenic at this stage so ignore 
 a.dash<-Gene.Names=="-"
sum(a.dash)
Gene.Names[a.dash]<-NA

} # no gene > 0
## indels[a.dash,][1:5,1:50]

## final.names<-indels[a.dash,"knownGene::gene"]
## final.names<-gsub("\\(dist=\\d*\\)","",final.names)
## final.names<-gsub("\\(dist=NONE\\)","",final.names)
## final.names[grepl("^NO_ANNOVAR",final.names)]<-"-"

## indels[a.dash,"Gene.Names"]<-final.names
## a.dash<-indels[,"Gene.Names"]=="-"
## sum(a.dash)
## indels[a.dash,c("chr","start")]
## final.names<-build.key(indels[a.dash,],c("chr","start"))
## indels[a.dash,"Gene.Names"]<-final.names

## unannotated.hits<-a.dash

## all.genes<-sort(table(indels[,"Gene.Names"]),decreasing=TRUE)
## all.genes[1:30]
...
}else if(affection.status.file=="coverage.file" | affection.status.file=="coverage_file"){ ## use a covege file and set as affected by default
  
 sample.sheet.full<-read.delim(paste(sheet.dir,the.sample.sheet,sep="/"),header=T,sep=",",fill=TRUE,stringsAsFactors=FALSE)#
colnames(sample.sheet.full)[colnames(sample.sheet.full)=="Project"]<-"SampleProject"
colnames(sample.sheet.full)[colnames(sample.sheet.full)=="Sample"]<-"ParticipantCode"
if( !("AffectionStatus" %in% colnames(sample.sheet.full))){
  AffectionStatus<-rep("2",times=dim(sample.sheet.full)[1])
  sample.sheet.full<-cbind(sample.sheet.full,  AffectionStatus)
}
   if( !("MaternalID" %in% colnames(sample.sheet.full))){
  MaternalID<-rep(".",times=dim(sample.sheet.full)[1])
  sample.sheet.full<-cbind(sample.sheet.full,  MaternalID)
}
   if( !("PaternalID" %in% colnames(sample.sheet.full))){
  PaternalID<-rep(".",times=dim(sample.sheet.full)[1])
  sample.sheet.full<-cbind(sample.sheet.full,  PaternalID)
}

}else if(affection.status.file=="no.file" | affection.status.file=="no_file"){
  restrict.family<-c("ALL")
  sample.sheet.full<-cbind(restrict.family,restrict.family,all.sample.labels,".",".","-9",2)
colnames(sample.sheet.full)<-c("SampleProject","FamilyCode","ParticipantCode","PaternalID","MaternalID","Sex","AffectionStatus")
}else{
sample.sheet.full<-read.delim(paste(sheet.dir,the.sample.sheet,sep="/"),header=T,sep=",",fill=TRUE,stringsAsFactors=FALSE)
...
genotypes<-indels[,paste(the.samples,"GT",sep=".")]  ##### WAS the.samples ##### WAS LEO pharma the.samples.other
allele.depths<-indels[,paste(the.samples,"AD",sep=".")] ##### WAS the.samples ##### WAS for LEO pharma the.samples.other
...
indels.order.GT.AN<-c(expand.labels.to.samples(c(the.affected,the.Unaffected,the.unknown,the.samples.other),c("GT","AD"),paste.after=TRUE))
...
indels.order.2nd<-expand.labels.to.samples(c(the.affected,the.Unaffected,the.unknown,the.samples.other),c("DP","GQ","PL"),paste.after=TRUE) ##"DP","GQ","PL" rarely used removed in unwanted.cols
...
unwanted.columns<-c("refGene" ,"knownGene", "ensGene","gene.table",
                    ...
                    expand.labels.to.samples(c(the.affected,the.Unaffected,the.unknown),c("PL"),paste.after=TRUE),
...
}}  # exists("family.name.prefix"
  
#dim(summary)
if(exists("quality.thresh")){rm("quality.thresh")}
...
gt.labels<-expand.labels.to.samples(c("GT"),sample.labels)
gq.labels<-expand.labels.to.samples(c("GQ"),sample.labels)
i<-200

frequency<-c(0,0.001,0.005,0.01,0.025)
f.col<-paste("MAF.lt:",frequency,sep="")

i<-200

for (i in 1:length(gt.labels)){

has.geno<-summary[,"FILTER"]=="PASS"  &  (summary[,gt.labels[i]] %in% c("0/1","1/1") & !is.na(summary[,gt.labels[i]])) & (as.numeric(summary[,gq.labels[i]])>20 & !is.na(summary[,gt.labels[i]]))

vep<-table(summary[has.geno,  "Consequence.Embl" ])
refseq<-table(summary[has.geno,  "refGene::location" ])
#tapply(summary[has.geno,  "Consequence.Embl" ],vep.types,length)

sum(!(names(vep) %in% vep.types))
sum(!(names(refseq) %in% possible.mutations))

vep<-vep[vep.types]
refseq<-refseq[possible.mutations]

names(vep)<-vep.types
names(refseq)<-possible.mutations

vep[is.na(vep)]<-0
refseq[is.na(refseq)]<-0


refseq.all<-c(sample.labels[i],"ALL",as.character(summary[1,"chr"]),refseq)
vep.all<-c(sample.labels[i],"ALL",as.character(summary[1,"chr"]),vep)

names(vep.all)[c(1,2,3)]<-c("Sample","Freq","chr")
names(refseq.all)[c(1,2,3)]<-c("Sample","Freq","chr")

vep.all
refseq.all
has.geno.all<-has.geno
iff<-1
for(iff in 1:length(f.col)){

has.geno<-has.geno.all & summary[, f.col[iff] ] 
...
refseq<-c(sample.labels[i],f.col[iff],as.character(summary[1,"chr"]),refseq)
vep<-c(sample.labels[i],f.col[iff],as.character(summary[1,"chr"]),vep)

names(vep)[c(1,2,3)]<-c("Sample","Freq","chr")
names(refseq)[c(1,2,3)]<-c("Sample","Freq","chr")


vep.all<-rbind(vep.all,vep)
refseq.all<-rbind(refseq.all,refseq)
}

if(is.null(vep.all.samp)){
  
vep.all.samp<-vep.all
refseq.all.samp<-refseq.all
}else{
vep.all.samp<-rbind(vep.all.samp,vep.all)
refseq.all.samp<-rbind(refseq.all.samp,refseq.all)
...
save(list=c("vep.all.samp","refseq.all.samp"),file="syno.counts.RData")
getwd()
vep.all.samp[1:10,1:10]
refseq.all.samp[1:10,1:10]
the.chr<-unique(refseq.all.samp[,"chr"])


i<-1
refseq.all.samp.sum.all<-as.numeric(refseq.all.samp[refseq.all.samp[,"chr"] %in% the.chr[1],possible.mutations])
dim(refseq.all.samp.sum.all)<-c(sum(refseq.all.samp[,"chr"] %in% the.chr[1]),length(possible.mutations))
refseq.all.samp.sum.all[1:5,]


for( i in 2:length(the.chr)){
refseq.all.samp.sum<-as.numeric(refseq.all.samp[refseq.all.samp[,"chr"] %in% the.chr[i],possible.mutations])
dim(refseq.all.samp.sum)<-c(sum(refseq.all.samp[,"chr"] %in% the.chr[i]),length(possible.mutations))
refseq.all.samp.sum[1:5,]
refseq.all.samp.sum<-refseq.all.samp.sum+refseq.all.samp.sum
