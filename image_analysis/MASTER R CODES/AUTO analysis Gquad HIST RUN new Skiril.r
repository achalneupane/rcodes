...
a.model.G2<-function(x,A,B,g1.peak.posn,g2.peak.posn,g1.sd,g2.sd,k0,k1,k2,k3,g1.sd.inter,g2.sd.inter,static){
 ...
 a.model.signal<-function(x,A,background.peak.posn,background.sd,k1,background.sd.inter,static){
   
              ak.curve<-aspline(static$x, static$y,x,method="improved",degree=3)
              ak.curve$y[!is.finite(ak.curve$y)]<-0.0
              
              A*dnorm(x,mean=background.peak.posn,sd=background.sd, log=FALSE) + abs(ak.curve$y)*pnorm( sqrt(2)*( ((x-background.peak.posn)/background.sd.inter)-k1), lower.tail=TRUE, log.p=FALSE)
            }

 a.model.red<-function(x,A,background.peak.posn,background.sd,k1,background.sd.inter,static){
   
              ak.curve<-aspline(static$x, static$y,x,method="improved",degree=3)
              ak.curve$y[!is.finite(ak.curve$y)]<-0.0
              abs(ak.curve$y)*pnorm( sqrt(2)*( ((x-background.peak.posn)/background.sd.inter)-k1), lower.tail=TRUE, log.p=FALSE)
            }
...
### jane2
setwd("/media/scratch/Data/Cellomics/Jane2")
## the.files<-dir(getwd())
## the.files<-the.files[grep(".TXT",the.files)]
## file.list<-c("plate_jane.1")

file.list<-c("plate.MUS.a.1234","plate.MUS.a.5678","plate.MUS.b.1234","plate.MUS.b.5678","plate.MUS.c.1234","plate.MUS.c.5678")
...
########Kiril 2010
setwd("/media/scratch/Data/Cellomics/Kiril")
plate.numbers<-c(1:15,17:20,25:34)  #20
file.list=paste("Kiril_Plate",plate.numbers,sep="")
## file.name.list<-file.list

########Joseph 2010
setwd("/media/scratch/Data/Cellomics/Joseph")
plate.numbers<-c(1:18,21:24)  #20
file.list=paste("plate_",plate.numbers,sep="")
## file.name.list<-file.list
...
## siFB2
max.g1.posn<-7500 #6000 10000-1.1 7000-3.1 for ca3 : 20000 before
min.g1.posn<-2500  #2750 5900-1.1 for Ca3 
expected.g1.posn<-5000
max.ObjectTotalIntenCh1<-18000


## siFB1 and 3
model.background.red<-TRUE
k1.use=5
do.trace<-FALSE # tarce for non-linear fit
robust.shaver<-1  # 0 or 1  is used to shave very close/close Ca screens when standard method fails, 0 to use standard method
max.cells.per.field<-4000 # ususally 5000  ## usual 2000 but some wells measured twice
min.green.cells<-50
min.red.cells<-30
min.green.threshold<-50
...
## red.total.thresh<-7500 # cut of for  average red signal Now modelled via background subtraction

red.yrange.on.plot<-25000
use.Edu.as.Sphase<-TRUE
min.red.cells.for.Sphase<-50 ## lower than this and it will model the S-phase
g2.over.g1.min<-1.99  #1.9 before
g2.over.g1.max<-2.06  #2.35 before
...
do.trace<-FALSE # tarce for non-linear fit
robust.shaver<-1  # 0 or 1  is used to shave very close/close Ca screens when standard method fails, 0 to use standard method
max.cells.per.field<-2000 # ususally 5000  ## usual 2000 but some wells measured twice
min.green.cells<-30
min.red.cells<-30
min.green.threshold<-32
max.g1.posn<-23000 #6000 10000-1.1 7000-3.1 for ca3 : 20000 before max.g1.posn*g2.over.g1.max
min.g1.posn<-9000  #2750 5900-1.1 for Ca3 
expected.g1.posn<-15000 # 3000 8200-1.1 for Ca3
max.ObjectTotalIntenCh1<-50000 # used 30000 in past bust is too low sometimes BEST DECIDED after first fit
double.exposure<-FALSE
use.high<-FALSE  # TRUE for Ca3 FALSE for CA2 highest exposure
two.color<- TRUE # if true red and green channels SIRNA has only one - false
red.mean.thresh<-30 # cut of for  average red signal
## red.total.thresh<-7500 # cut of for  average red signal Now modelled via background subtraction
...
Kiril
## siFB1 and 3
model.background.red<-FALSE
k1.use=5
do.trace<-FALSE # tarce for non-linear fit
robust.shaver<-0  # 0 or 1  is used to shave very close/close Ca screens when standard method fails, 0 to use standard method
max.cells.per.field<-2000 # ususally 5000  ## usual 2000 but some wells measured twice
min.green.cells<-30
min.red.cells<-30
min.green.threshold<-50
max.g1.posn<-37000 #6000 10000-1.1 7000-3.1 for ca3 : 20000 before max.g1.posn*g2.over.g1.max
min.g1.posn<-14000  #2750 5900-1.1 for Ca3 
expected.g1.posn<-28000 # 3000 8200-1.1 for Ca3
max.ObjectTotalIntenCh1<-90000 # used 30000 in past bust is too low sometimes BEST DECIDED after first fit
double.exposure<-TRUE
use.high<-FALSE  # TRUE for Ca3 FALSE for CA2 highest exposure
two.color<- TRUE # if true red and green channels SIRNA has only one - false
red.mean.thresh<-50 # cut of for  average red signal
## red.total.thresh<-7500 # cut of for  average red signal Now modelled via background subtraction

red.yrange.on.plot<-120000
use.Edu.as.Sphase<-TRUE
min.red.cells.for.Sphase<-50 ## lower than this and it will model the S-phase
g2.over.g1.min<-1.99  #1.9 before
g2.over.g1.max<-2.06  #2.35 before
...
min.red.cells<-30
min.green.threshold<-32
max.g1.posn<-22000 #6000 10000-1.1 7000-3.1 for ca3 : 20000 before max.g1.posn*g2.over.g1.max
min.g1.posn<-8000  #2750 5900-1.1 for Ca3 
expected.g1.posn<-16000 # 3000 8200-1.1 for Ca3
max.ObjectTotalIntenCh1<-60000 # used 30000 in past bust is too low sometimes BEST DECIDED after first fit
double.exposure<-FALSE
use.high<-FALSE  # TRUE for Ca3 FALSE for CA2 highest exposure
two.color<- TRUE # if true red and green channels SIRNA has only one - false
red.mean.thresh<-50 # cut of for  average red signal
...
for (iBarCodes in 1:length(barCodes)){
  ...
  ## if(barCodes[iBarCodes]=="si FB2_3.1" | barCodes[iBarCodes]=="si FB2_3.2" ){next}
...
the.cells<-cells[cells[,"BarCode"]==barCodes[iBarCodes],]
 ...
 file.plate.name<-paste("plate",barCodes[iBarCodes],sep="_") #typical

  ########$$$$$$$$$$$$ special for Kiril
  file.plate.name<-file.name #for kirl and joseph only
  #######$$$$$$$$$
...
if(double.exposure){number.cols<-c(2:dim(the.cells)[2])}else{number.cols<-c(3:dim(the.cells)[2])}
  ...
  ########$$$$$$$$$$$$ special for Kiril
number.cols<-c(3:dim(the.cells)[2])#for kirl and joseph only
  #######$$$$$$$$$
...
if(model.background.red){
aden<-density(test[,yaxis]  )

##########################################get an estimate for the red backgroud but fitting it to a gaussian to assist in finding red cells
a.peak<-peaks(aden$y,span=9)
a.quant<-quantile(test[,"TotalIntenCh3"]  )
...
## points(aden$x[a.peak],aden$y[a.peak],pch=23,col="red") ### found peak
 ...
 potential.peaks<-aden$y[a.peak]
  ...
  background.peak.posn<-aden$x[a.peak][highest.place]
  background.peak.height<-aden$y[a.peak][highest.place]
  background.peak.place<-sum(aden$x <= background.peak.posn)
  background.sd<-(background.peak.posn-aden$x[1])/4 # initial guess
  background.sd.inter<-background.sd

 
  A<- background.peak.height/dnorm( background.peak.posn,mean= background.peak.posn,sd= background.sd, log=FALSE) # initial guess
  k1<-k1.use # k1

  ## points(background.peak.posn,background.peak.height,pch=23,col="blue")

static<-data.frame(x=aden$x,y=aden$y)
to.fit<-data.frame(y=aden$y, x=aden$x)
rm(sig.the.fit)
 try(sig.the.fit<-nls(y~(a.model.signal(x,A,background.peak.posn,background.sd,k1,background.sd.inter,static) ) 
          ,data=to.fit
          ,start=list(A=A,background.peak.posn=background.peak.posn, background.sd=background.sd, k1=k1,background.sd.inter=background.sd.inter)
          ,lower=c(A*0.1,    background.peak.posn-0.05*background.sd, background.sd*0.8, k1-0.1,background.sd/10)
          ,upper=c(A+0.5*A,  background.peak.posn+0.05*background.sd, background.sd*1.2,k1+5,background.sd*7)
          ,trace=do.trace
          ,algorithm="port"
          ,control=list(maxiter=1000, minFactor=1/2048,tol=1e-4, warnOnly = TRUE) ),silent=TRUE)
 
 if(!exists("sig.the.fit")){red.total.thresh<-a.quant["75%"]}else{




  the.coefs<-coef(sig.the.fit)
  A<-as.numeric(the.coefs["A"])
  background.peak.posn<-as.numeric(the.coefs["background.peak.posn"])
  background.sd<-as.numeric(the.coefs["background.sd"])
  k1<-as.numeric(the.coefs["k1"])

## curve(A*dnorm(x,mean=background.peak.posn,sd=background.sd, log=FALSE),add=TRUE, col="purple",lwd=2,lty="dashed")
## curve( a.model.red(x,A,background.peak.posn,background.sd,k1,background.sd.inter,static), add=TRUE, col="red",lwd=2,lty="dashed")
## curve( a.model.signal(x,A,background.peak.posn,background.sd,k1,background.sd.inter,static), add=TRUE, col="green",lwd=2,lty="dashed")
## background.peak.posn+ k1.use* background.sd
## abline(v=background.peak.posn+ k1.use* background.sd)
############
            


if(background.peak.posn > a.quant["75%"]){print("WARNING:backgroup peak greater than 75% qantile-unexpected");background.peak.posn <- a.quant["75%"]}
red.total.thresh<-background.peak.posn+ k1.use* background.sd
 
}


 ##################################################### gor the background now do the fitting
red<-test[,"TotalIntenCh3"]>=red.total.thresh # red<-rep(FALSE,times=dim(test)[1])
}else{red<-rep(FALSE,times=dim(test)[1])}
...
red[rownames(test[!red,])[red.trimmed]]<-TRUE
red.c[i,j]<-sum(red)
the.model.have<-the.model # backtrack one

the.model<-try(lmrob(TotalIntenCh3~ObjectTotalIntenCh1,data=test,subset=c(1:dim(test)[1])[!red]),silent=TRUE)

if(inherits(the.model, "try-error")){print("fitting error inn lmrob"); the.model<-the.model.have; lm.red.coverg[i,j]<-"FAIL" }else{lm.red.coverg[i,j]<-the.model$converged}
lm.red.slope[i,j]<-coef(the.model)[2]
lm.red.inter[i,j]<-coef(the.model)[1]

if(lm.red.coverg[i,j]=="FAIL"){
  red.trimmed<- the.model$lts.wt==0 &  residuals(the.model)>0  & test[!red,"AvgIntenCh3" ]>= red.mean.thresh
}else{
red.trimmed<- (abs(weights(the.model)) < 1/length(weights(the.model))) &  residuals(the.model)>0  & test[!red,"AvgIntenCh3"] >= red.mean.thresh
}
...
## plot((test[,xaxis]),(test[,yaxis]),pch=20,cex=0.5,xlab="Total Intensity of DAPI",ylab="Total Intensity of Edu",font=2,font.lab=2,cex.axis=2,cex.lab=3,cex.main=2.0,ylim=c(0,55000),xlim=c(0,55000))
## if(!inherits(the.model, "try-error")){abline(coef(the.model),lty=10,col="red",lwd=3)}
## points(test[red,xaxis],test[red,yaxis],col="red",pch=21,cex=0.8)
## savePlot("regression figure.tiff",type="tiff")
## savePlot("regression figure.jpeg",type="jpeg")

## abline(h=0,lty=10,lwd=2)
## points(test[red,xaxis],test[red,yaxis],col="red",pch=19,cex=0.4)
...
#print(as.character(the.model$converged))
## lmrob.control(max.it=50,rel.tol=1e-7,trace.lev=0)

the.model2 <-try(lmrob(VarIntenCh2~AvgIntenCh2+I(AvgIntenCh2^2),data=test,subset=c(1:dim(test)[1])[mostly.green], ),silent=TRUE)

if(sum(inherits(the.model2, "try-error") | is.na((the.model2$cov)))>0 ){green<-mostly.green}else{
...
green<-(test[,yaxis]<=  ((coef(the.model)[3]+1*sqrt(diag(the.model2$cov))[3])* test[,xaxis]^2  +  (coef(the.model)[2]+0.25*sqrt(diag(the.model2$cov))[2])* test[,xaxis]+ coef(the.model)[1]+ 2*sqrt(diag(the.model2$cov))[1]    ) & is.finite(test[,xaxis])  &  is.finite(test[,yaxis]) & test[,xaxis]>=log2(min.green.threshold)  ) #  joseph

if( sum(mostly.green) < sum(green) ){green<-mostly.green} ##flipped to < for kiril was > for joseph > for jane
...
plot((test[,xaxis]),(test[,yaxis]),pch=20,cex=0.1,xlab="log2( Average Green Signal )",ylab="log2( Varience Green Signal )",main="",font.lab=2,cex.lab=1.5,xlim=c(0,11)) 
...
############# for publication
## par(mar=c(5,6.0,2.5,5.1),mgp=c(3.5,1,0))
## plot((test[,xaxis]),(test[,yaxis]),pch=20,cex=0.5,xlab="log2( Average GFP Signal )",ylab="log2( Varience GFP Signal )",main="",font=2,font.lab=2,cex.axis=2,cex.lab=3,cex.main=2.0,xlim=c(2,11)) 
## ## plot((test[,xaxis]),(test[,yaxis]),pch=20,cex=0.5,xlab="Total Intensity of DAPI",ylab="Total Intensity of Edu",font=2,font.lab=2,cex.axis=2,cex.lab=3,cex.main=2.0,ylim=c(0,55000),xlim=c(0,55000))
## points(test[green,xaxis],test[green,yaxis],col="green",pch=21,cex=0.8)
## points(test[red,xaxis],test[red,yaxis],col="red",pch=20,cex=0.5)
## order.in<-order(test[well.rows.sub,xaxis])
## lines(test[well.rows.sub[order.in],xaxis],the.model$fit[order.in],col="magenta",lwd=3)

## points(test[red,xaxis],test[red,yaxis],col="red",pch=21,cex=0.8)
## savePlot("regression figure green.tiff",type="tiff")
## savePlot("regression figure green.jpeg",type="jpeg")
...
######################## do DNA histgrams #############################
 ...
 if(sum(!green)>4){adenNG<-density(test[!green,xaxis])} else{print("WArning no not green cells");green[1:4]<-TRUE} # provide warning but keep going
 if(sum(!red)>4){ adenNR<-density(test[!red,xaxis])}else{print("WArning no not red cells");red[1:4]<-TRUE} # provide warning but keep going
                                                         
  if(sum(green)<min.green.cells){adenG<-adenNG}else{adenG<-density(test[green,xaxis])} # avoid error if no green cells
  if(sum(green & red)<min.green.cells){adenGR<-adenNG}else{adenGR<-density(test[green & red,xaxis])} # avoid error if no green cells
 ...
 the.max.range<-max(aden$y,adenG$y,adenNG$y,adenR$y,adenNR$y)
  ...
  plot(aden,lwd=2,col="black",main="",font.lab=2,cex.lab=1.5,ylim=c(0,the.max.range),xlim=c(0,max.ObjectTotalIntenCh1))
 ...
 if(two.color){
  ...
  lines(adenR$x,percent.R*adenR$y,col="bisque3",lwd=2)
  lines(adenGR$x,percent.R*adenGR$y,col="coral",lwd=2)
  leg.txt<-c("Cell Types:", "All","Green","Not Red","Red","Green and Red")
  label.colors<-c("black","black","green","grey50","bisque3","coral")
  the.lty<-c(-1,1,1,1,1,1)

  fit.final.txt<-c("--------","Final Fits:","All","Green","Not Green")
  the.final.colors<-c("Black","Black","grey50","green","grey34" )
  the.final.lty<-c(-1,-1,3,3,3)

                                          
}else{
  lines(adenR$x,adenR$y,col="bisque3",lwd=2)
   lines(aden$x,aden$y,col="black",lwd=2)
  lines(adenNR$x,adenNR$y,col="grey50",lwd=2)
  leg.txt<-c("Cell Types:", "All","Not Red","Red")
  label.colors<-c("black","black","grey50","bisque3")
  the.lty<-c(-1,1,1,1)

  fit.final.txt<-c("--------","Final Fits:","All","Not Red")
  the.final.colors<-c("Black","Black","grey50","grey34" )
  the.final.lty<-c(-1,-1,2,2)

  
}
fit.colors<-c("Black","Black","purple","violet","red","turquoise4","turquoise")
fit.txt<-c("--------","Fitted using all cells:","G1","G2","S",">4N","<2N")
fit.lty<-c(-1,-1,2,2,2,2,2)

leg.txt<-c(leg.txt,fit.txt,fit.final.txt)
label.colors<-c(label.colors,fit.colors,the.final.colors )
the.lty<-c(the.lty,fit.lty,the.final.lty)
legend(x=0,the.max.range,legend=leg.txt,text.col=label.colors,pch=-1,lty=the.lty,bty="n",cex=1.1,col=label.colors,lwd=2)
 ...
 if(g1.peak.posn >= max.g1.posn ){ g1.peak.posn<-max.g1.posn  }
  ...
  g1.region<-adenNR$x<=(g1.peak.posn+0.55*g1.peak.posn)
  ...
  the.fit<-nls(y~A*dnorm(x,mean=g1.peak.posn,sd=g1.sd, log=FALSE),
               ...
               ,lower=c(A/10, min.g1.posn, g1.sd/2)
               ,upper=c(A*10, max.g1.posn, g1.sd*2),
                ,trace=do.trace
               ,algorithm="port"
 ...
 if(g1.peak.posn >= max.g1.posn ){ g1.peak.posn<-g1.peak.posn.ori }  # above fit on G1 only is bollocks but keep g1.sd and A
  ...
  if((g2.peak.posn>=g2.over.g1.max*g1.peak.posn) |  (g2.peak.posn <= g2.over.g1.min*g1.peak.posn) | is.na(adenNR$x[a.peak][second.highest.place]) ){  ## can't get a good peak
    ...
    points(g2.peak.posn,g2.peak.height,pch=23,col="darkorchid")
...
if(use.den){
  ...
  if(profiles[iprofile]=="adenG"){if(sum(red & green)==0){redandgreen<-1}else{redandgreen<-sum(red & green)} ;scaleR.use <-redandgreen/sum(green) ; scaleR.min<- scaleR.use-0.05 ; if(scaleR.min<=0){scaleR.min<-0.01} ; scaleR.max<- scaleR.use+0.05; if(scaleR.max>=0.96){scaleR.max<-0.96}} # red and green
  if(profiles[iprofile]=="adenNG"){ scaleR.use <-sum(red & !green)/sum(!green) ; scaleR.min<- scaleR.use-0.05 ; if(scaleR.min<=0){scaleR.min<-0.01} ; scaleR.max<- scaleR.use+0.05; if(scaleR.max>=0.96){scaleR.max<-0.96}} # red no green
  ...
  try(new.the.fit<-nls(y~(a.model(x,A,B,g1.peak.posn,g2.peak.posn,g1.sd,g2.sd,k0,k1,k2,k3,g1.sd.inter,g2.sd.inter,static,use.den,adenR,scaleR) ) 
          ...
          ,lower=c(A*0.01,    B*0.01,  g1.peak.posn-0.05*g1.sd, g1.sd*0.8,  g2.peak.posn-g2.over.g1.refit*g2.sd ,  g2.sd*0.8, 0.48, 0.49, scaleR.min)
 ...
 if(!exists("new.the.fit")){ new.the.fit<-the.fit; DNA.fitting[[eval(profiles[iprofile])]][i,j]<-"FAIL-singular"
  ...
  try(new.the.fit<-nls(y~(a.model(x,A,B,g1.peak.posn,g2.peak.posn,g1.sd,g2.sd,k0,k1,k2,k3,g1.sd.inter,g2.sd.inter,static,use.den,adenR,scaleR)  )
          ,data=new.to.fit       
          ,start=list(A=A,B=B,g1.peak.posn=g1.peak.posn, g1.sd=g1.sd, g2.peak.posn=g2.peak.posn, g2.sd=g2.sd, k0=k0, k1=k1, k2=k2, k3=k3, g1.sd.inter=g1.sd.inter, g2.sd.inter=g2.sd.inter)
          ,lower=c(A*0.01,   B*0.01,   g1.peak.posn-0.05*g1.sd, g1.sd*0.8, g2.peak.posn-g2.over.g1.refit*g2.sd ,  g2.sd*0.8, 0.49, 0.49, -2.1,  0.29, g1.sd.inter*0.8, g2.sd.inter*0.8)
...
leg.txt<-c(paste("Red=",sum(red),sep=""),
             ...
             paste("Not Green: % Red=",round(100*sum(red & !green)/sum(!green),1),sep=""),
             paste("Green: % Red=",round(100*sum(red & green)/sum(green),1),sep=""),
  
             "--------------------------",
             "RATIOs FOR  Green Cells",
               paste("(S+G2)/G1 =",round(DNA.G1andG2$adenG[i,j],2),sep=""),
                paste("(S+G2+ >4N)/G1 =",round(DNA.aboveG1$adenG[i,j],2),sep=""),
               paste("S =",round( DNA.S$adenG[i,j],2),sep=""),
               paste("> 4N =",round(DNA.gt4$adenG[i,j],2),sep=""),
               paste("< 2N =",round(DNA.lt2$adenG[i,j],2),sep=""),
               "--------------------------",
             "RATIOs FOR  Green/NotGreen Cells",
...
legend(x=g1.peak.posn*2.3,the.max.range,legend=leg.txt,text.col=c("black","black","black","black","black","brown","grey50","forestgreen",
                                                         "black",
                                                         "black","red","orange4","black","black","black",
                                                         "black",
                                                         "black","red","orange4","black","black","black",
                                                         "black","black"),pch="",bty="n",cex=1.10)
}else{
leg.txt<-c(paste("Red=",sum(red),sep=""),
...
legend(x=g1.peak.posn*2.3,the.max.range,legend=leg.txt,text.col=c("red","grey35","red","black","black","red","orange4","black","black","black","orange","black","black"),pch="",bty="n",cex=1.10)
