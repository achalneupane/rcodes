...
## max.g1.posn<-27000 #6000 100100-1.1 7000-3.1 for ca3 : 20000 before
...
### jane-validation
setwd("/media/scratch/Data/Cellomics/Jane-validation")
## the.files<-dir(getwd())
## the.files<-the.files[grep(".TXT",the.files)]
## file.list<-c("plate_jane.1")

file.list<-c("Plate.1","Plate.2","Plate.3","Plate.4")
...
max.g1.posn<-29000 #6000 10000-1.1 7000-3.1 for ca3 : 20000 before max.g1.posn*g2.over.g1.max
min.g1.posn<-14000  #2750 5900-1.1 for Ca3 
expected.g1.posn<-23000 # 3000 8200-1.1 for Ca3
max.ObjectTotalIntenCh1<-80000 # used 30000 in past bust is too low sometimes BEST DECIDED after first fit
...
## siFB1 and 3
k1.use=4
do.trace<-FALSE # trace for non-linear fit
robust.shaver<-1  # 0 or 1  is used to shave very close/close Ca screens when standard method fails, 0 to use standard method
max.cells.per.field<-2000 # ususally 5000  ## usual 2000 but some wells measured twice
min.green.cells<-30
min.red.cells<-30
...
Jane-validation
## siFB1 and 3
##### if mkae k1.use lower (5ish) then done use high proliferation set to 0.9
k1.use=15 ## how many stand devation away from the backgroup should red cells be defined
high.proliferation<-0.3 # is high proliferation then need to do more robust regression goes to method 1
low.proliferation<-0.1 # low proliferation can confount the background
do.trace<-FALSE # tarce for non-linear fit
robust.shaver<-0  # 0 or 1  is used to shave very close/close Ca screens when standard method fails, 0 to use standard method
max.cells.per.field<-1000 # ususally 5000  ## usual 2000 but some wells measured twice ## 1000 here case is 20x
min.green.cells<-30
min.red.cells<-30
min.green.threshold<-128 ##used 32 first time
max.g1.posn<-40000 #6000 10000-1.1 7000-3.1 for ca3 : 20000 before max.g1.posn*g2.over.g1.max
min.g1.posn<-9000  #2750 5900-1.1 for Ca3 
expected.g1.posn<-25000 # 3000 8200-1.1 for Ca3
max.ObjectTotalIntenCh1<-70000 # used 30000 in past bust is too low sometimes BEST DECIDED after first fit
double.exposure<-FALSE # Works for a single exposure DUMPED as a DOUBLE also
use.high<-FALSE  # TRUE for Ca3 FALSE for CA2 highest exposure
two.color<- TRUE # if true red and green channels SIRNA has only one - false
red.mean.thresh<-30 # cut of for  average red signal
## red.total.thresh<-7500 # cut of for  average red signal Now modelled via background subtraction

red.yrange.on.plot<-150000
use.Edu.as.Sphase<-TRUE
min.red.cells.for.Sphase<-50 ## lower than this and it will model the S-phase
g2.over.g1.min<-1.99  #1.9 before
g2.over.g1.max<-2.06  #2.35 before
...
rm(sig.the.fit)
 try(sig.the.fit<-nls(y~(a.model.signal(x,A,background.peak.posn,background.sd,k1,background.sd.inter,static) ) 
          ...
          ,lower=c(A*0.1,    background.peak.posn-0.05*background.sd, background.sd*0.8, 3.5,background.sd/10)
          ,upper=c(A+0.5*A,  background.peak.posn+0.05*background.sd, background.sd*1.2,7,background.sd*7)
          ,trace=do.trace
          ,algorithm="port"
...
## abline(v=red.total.thresh,col="blue")
############
...
got.high.proliferation<-(sum(red)/dim(test)[1] > high.proliferation)
got.low.proliferation<-(sum(red)/dim(test)[1] < low.proliferation)
if(got.high.proliferation | got.low.proliferation ){
  ###repeat again
if(got.low.proliferation ){red<-rep(FALSE,times=dim(test)[1]); names(red)<-rownames(test) } ## can shave too close
  
  got.high.proliferation<-FALSE
  got.high.proliferation<-FALSE
...
### go againing with more regression
  got.high.proliferation<-FALSE
  got.high.proliferation<-FALSE
 
##   Use this second iteration to get really close!!- espect for high proliferators
the.model<-ltsReg(TotalIntenCh3~ObjectTotalIntenCh1,data=test,subset=c(1:dim(test)[1])[!red])
...
points(test[red,xaxis],test[red,yaxis],col="red",pch=19,cex=0.5)

##  abline(h=red.total.thresh,col="blue")
## abline(h=red.total.thresh,col="blue")
...
resid.quant<-quantile(the.model$residuals,c(0.95)) ##0.85 is also ued in some cases
...
plot((test[,xaxis]),(test[,yaxis]),pch=20,cex=0.1,xlab="log2( Average Green Signal )",ylab="log2( Varience Green Signal )",main="",font.lab=2,cex.lab=1.5,xlim=c(0,11),ylim=c(0,11)) 
...
legend(x=0,the.max.range,legend=leg.txt,text.col=label.colors,pch=-1,lty=the.lty,bty="n",cex=1.1,col=label.colors,lwd=2)
 ...
 if(g1.peak.posn >= max.g1.posn ){ g1.peak.posn<-max.g1.posn  }
  ...
  the.fit<-nls(y~A*dnorm(x,mean=g1.peak.posn,sd=g1.sd, log=FALSE),
               ...
               ,lower=c(A/10, min.g1.posn, g1.sd/4) ##was 2 before jane validation
               ,upper=c(A*10, max.g1.posn, g1.sd*4),
                ,trace=do.trace
               ,algorithm="port"
...
## lines(to.fit$x,A*dnorm(to.fit$x,mean=g1.peak.posn,sd=g1.sd, log=FALSE),col="red",lwd=2)
