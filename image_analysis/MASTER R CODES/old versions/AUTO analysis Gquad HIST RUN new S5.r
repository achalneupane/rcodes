...
do.trace<-FALSE # tarce for non-linear fit
robust.shaver<-5  # 2 / 1 is used to shave very close/close Ca screens when standard method fails, 0 to use standard method
...
red.total.thresh<-5000 # cut of for  average red signal
...
## points(test[red,xaxis],test[red,yaxis],col="orange",pch=19,cex=0.4)



if(robust.shaver > 1){
  i.shave<-2
  intercept<-coef(the.model)[1]
  coef.old<-coef(the.model)
    coef<-coef+100000

  
  intercept.old<-intercept+1000
  coef<-coef+100000
  
  red.old.num<-sum(red)
  red.num<-red.old.num+100000
  ## print(coef(the.model))

 while( i.shave<=robust.shaver & (abs( (coef[2]*max.g1.posn + coef[1])) > median(test[,c("TotalIntenCh3")]) )  &  (abs( (coef[2]*max.g1.posn*g2.over.g1.max + coef[1])  - (coef.old[2]*max.g1.posn*g2.over.g1.max + coef.old[1]) )> 500) & ( coef[1]>0 | coef[2]>0)  ){


the.model.have<-the.model
red.old<-red
red.old.num<-sum(red)
## test[!red,c("TotalIntenCh3","ObjectTotalIntenCh1")]
...
## red.trimmed<-   residuals(the.model)>0 # & test[!red,"AvgIntenCh3" ]>= red.mean.thresh
...
red.num<-sum(red)
 ...
 ## print(coef(the.model))
 ## print( abs( (coef[2]*max.g1.posn*g2.over.g1.max + coef[1])  - (coef.old[2]*max.g1.posn*g2.over.g1.max + coef.old[1]) ))
 ## if(!inherits(the.model, "try-error")){abline(coef(the.model),lty=10,col="red",lwd=2)}
 ##  points(test[red,xaxis],test[red,yaxis],col="orange",pch=19,cex=0.4)
...
the.model<-the.model.have # backtrack one
## red<-red.old # backtrack one

## test[!red,c("TotalIntenCh3","ObjectTotalIntenCh1")]
## median(test[,c("TotalIntenCh3")])
## test[test[,c("TotalIntenCh3")]==0,"TotalIntenCh3"]<-1

the.model<-try(lmrob(TotalIntenCh3~ObjectTotalIntenCh1,data=test,subset=c(1:dim(test)[1])[!red]),silent=TRUE)
 ## if(inherits(the.model, "try-error")){red<-rep(FALSE,dim(test)[1])
...
new.the.fit<-nls(y~(a.model(x,A,B,g1.peak.posn,g2.peak.posn,g1.sd,g2.sd,k0,k1,k2,k3,g1.sd.inter,g2.sd.inter,static,use.den,adenR,scaleR) )
          ...
          ,lower=c(A*0.1,    B*0.1,  g1.peak.posn-0.05*g1.sd, g1.sd*0.8,  g2.peak.posn-g2.over.g1.refit*g2.sd ,  g2.sd*0.8, 0.5, 0.5, scaleR.min)
          ,upper=c(A+0.5*A, B+0.5*B, g1.peak.posn+0.05*g1.sd, g1.sd*1.2,  g2.peak.posn+g2.over.g1.refit*g2.sd,  g2.sd*1.2, 6, 5, scaleR.max)
          ,trace=do.trace
          ,algorithm="port"
          ,control=list(maxiter=1000, minFactor=1/2048,tol=1e-4, warnOnly = TRUE) )
