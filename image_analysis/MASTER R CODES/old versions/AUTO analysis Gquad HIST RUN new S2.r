...
min.red.cells<-30
min.green.threshold<-50
max.g1.posn<-15000 #6000 10000-1.1 7000-3.1 for ca3 : 20000 before
...
two.color<- FALSE # if true red and green channels SIRNA has only one - false
red.mean.thresh<-15 # cut of for  average red signal 
...
while( i.shave<=robust.shaver & (abs(intercept-intercept.old)> 150) ){
...
points(test[red,xaxis],test[red,yaxis],col="green",pch=19,cex=0.4)
...
the.fit<-nls(y~(a.model(x,A,B,g1.peak.posn,g2.peak.posn,g1.sd,g2.sd,k0,k1,k2,k3,g1.sd.inter,g2.sd.inter,static,use.den,adenR,scaleR) )
          ...
          ,lower=c(A-0.25*A, B-0.5*B, g1.peak.posn-0.5*g1.sd, g1.sd/2, g1.peak.posn*g2.over.g1.min,   g2.sd/2, 0.5,  0.5, scaleR.min)
          ,upper=c(A+0.25*A, B+0.5*B, g1.peak.posn+0.5*g1.sd, g1.sd*2, g1.peak.posn*g2.over.g1.max,  g2.sd*2, 6, 5,scaleR.max)
          ,trace=do.trace
          ,algorithm="port"
