...
## max.g1.posn<-10500 #9000 for ca3 : 20000 before
## min.g1.posn<-5900  #2750 for Ca3 
## expected.g1.posn<-8200 # 3000 for Ca3
## max.ObjectTotalIntenCh1<-20000 # used 30000 in past bust is too low sometimes BEST DECIDED after first fit
## double.exposure<-TRUE
## use.high<-FALSE  # highest exposure
## two.color<- FALSE # if true red and green channels SIRNA has only one 
## red.mean.thresh<-100 # cut of for  average red signal 
## red.yrange.on.plot<-75000
...
reads<-100000
if(double.exposure){keep<-c(3,5,6,10,11,17:dim(chromo)[2])}else{keep<-c(3,5,6,10,11,17:dim(chromo)[2])}  # columns to keep
...
#layout.show(nf)
plot((test[,xaxis]),(test[,yaxis]),pch=20,cex=0.1,xlab="Total Intensity of DAPI",ylab="EDU Intensity",main=paste(file.name," Well:",row.label.letter,col.label.number,sep=" "),font.lab=2,cex.lab=1.5,cex.main=2.0,ylim=c(0,red.yrange.on.plot))
...
######################## do DNA histgrams #############################
  ...
  if(sum(red)<min.red.cells){adenR<-adenNR}else{adenR<-density(test[red,xaxis])} # avoid error if no red cells

 if(!two.color){adenNG<-aden;adenG<-aden}
...
DNA.G2[i,j]<- g2.peak.posn
 ...
 curve( a.model.S(x,A,B,g1.peak.posn,g2.peak.posn,g1.sd,g2.sd,k0,k1,k2,k3,g1.sd.inter,g2.sd.inter,static), add=TRUE, col="red",lwd=2,lty="dashed")
...
leg.txt<-c(paste("Red=",sum(red),sep=""),
             ...
             paste("% Red=",round(100*sum(red)/length(red),2),sep=""),
             "--------------------------",
             "DNA histogram data (black curve)",
               paste("S =",round((  DNA.S$aden)[i,j],2),sep=""),
               ...
               "--------------------------",
               paste("Fit Quality: ALL[NotRed] =",round((DNA.fit.success$aden)[i,j],2),"[",round((DNA.fit.success$adenNR)[i,j],2) ,"]" ," : G2/G1=",round(DNA.G2[i,j]/DNA.G1[i,j],2),sep=""))
legend(x=expected.g1.posn*2.5,the.max.range,legend=leg.txt,text.col=c("red","grey35","red","black","black","red","orange4","black","black","black","grey50","black","black"),pch="",bty="n",cex=1.25)
}
