...
file.list<-c("plate_1","plate_2","plate_3")
#file.list<-c("plate_3")
file.name.list<-file.list
...
robust.shaver<-1  # 1 is used to shave very close when standard method fails, 0 to use standard method
max.cells.per.field<-5000
min.green.cells<-100
min.green.threshold<-32
max.g1.posn<-35000
min.g1.posn<-6000
expected.g1.posn<-20000
...
######################## do DNA histgrams #############################
  ...
  if(sum(red)<red.thresh){adenR<-adenNR}else{adenR<-density(test[red,xaxis])} # avoid error if no red cells
 ...
 if(two.color){
  ...
  lines(aden$x,adenR$y,col="red",lwd=2)
  lines(aden$x,adenNR$y,col="grey50",lwd=2)
}
