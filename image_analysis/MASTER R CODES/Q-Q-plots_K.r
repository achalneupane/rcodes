...
other.cells<-grepl("4",norm[,"plate"])
sum(wanted.screen & !other.cells)
norm<-norm[wanted.screen & !other.cells,]
unique(norm[,"plate"])
#####
...
if(the.score=="Z-all Green"){filter<- filter & plates.all[,"RG_expected"]>=50}
...
#### Kosyta define these functions see below
qq<- qq.data(data.in,distribution="norm",the.mean=the.mean,the.sd=the.sd,plot.it=FALSE)

my.qq.plot(data.in,distribution="norm",col="blue",xlab="Expected Score",ylab="Observed score",xlim=range(qq$x), ylim=range(data.in),main=paste("Screen:",the.screen,"with 95% confidence intervals",":",the.score,sep=" "),the.mean=the.mean,the.sd=the.sd,cex.lab=1.5,cex.axis=1.5,cex.main=1.5,cex=1.5)
...
points(qq$x[posns],qq$y[posns],col="pink",pch=19,cex=0.95)
...
## legend(x=-6,y=5,legend=c(toString(norm.control.list),toString(pos.control.list),toString(pos.control.list2),"Col:12 -highlight"),col=c("cyan","red","pink","gold"),pch=c(20,20,20,21),cex=1.5)

legend(x=0.1,y=0.4,legend=c(toString(norm.control.list),toString(pos.control.list),toString(pos.control.list2)),col=c("cyan","red","pink"),pch=c(20,20,20,21),cex=1.5)
