...
genome.build<-"hg19"
dbSNP.build<-"131"
vcf.type="mercury" # "annovar" "v4" "v3" "plink_assoc" "mercury"

bam.extension<-".ReCal.sort.bam"
combined.extension<-""
snp.extension<-".txt"  # "_snps.raw.vcf"
...
names(variant.types)<-c("mercury") ### define data type for when reading below
use.variants<-c("snp")
...
if(inherits(xx, "try-error")){indel.dir<-project.dir;print("ERROR Could not find Indel dir")}
...
if(vcf.type=="mercury")source("read.sample.files.MultiAllele.piecewise.Mercury.r")
