...
indel.new<- process.GATK.indels(indels,vcf.type,format.types,info.types,info.labels,info.class,num.cores)
  
#################### this is how to build a Grange object I'm not using this so will remove to save space
...
## values(data.gr)<-data[,!(colnames(data) %in% gr.core.ann)]
## assign(sample.grs[i],value=data.gr)
####################
assign(sample.grs[i],value=indels.new)
