...
### *** DO NOT PUT UQCCG.data & projecet below *UNLESS* running manualy. Should be set from => /home/mmarshall/PBSHOME/PaulGenotypingPipeline/SubmitProjectScript.qsub on CLUSTER *****
### ******************************************


##### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
## DO NOT SET HERE, SHOULD COME FROM THE CLuster qsub scripts  from args[2] above
# UQCCG.data<-"/mnt/UQCCG/Sequencing/Data/Sequence_Genotypes/"
#UQCCG.data<-"/home/mmarshall/PBSHOME/HiSeq/genotyping"

## DO NOT SET HERE, SHOULD COME FROM THE CLuster qsub scripts from args[3] above
...
analysis.dir<-paste(UQCCG.data,project,"Analysis",sep="/")
 ...
 affection.status<-"sample_sheet"
#### the.sample.sheet<-"SKDP_SampleSheet.csv" ### DO NOT SET HERE, SHOULD COME FROM THE CLuster qsub scripts args[4]
 restrict.family<-c()
...
filter.cols.novel<-c("NHBLI_6500_ANNOVAR_ALL","NHBLI_6500_ALL","NHLBI_5400_ALL","NHLBI_5400_EUR","NHLBI_5400_AFR","1000genome","1000genome_asian","1000genome_mine","snp137","snp137_clinical","snp137","CG69","EUR_ASN_AFR_INDEL","AOGC-NGS_ALL","AOGC-NGS_ALL_OLD","Chinese") ##
filter.cols.maf<-c("NHBLI_6500_ANNOVAR_ALL","NHBLI_6500_ALL","NHBLI_6500_EA","NHBLI_6500_AA","NHLBI_5400_ALL","1000genome","snp137","AOGC-NGS_ALL")
...
alt.allele.other.use<-TRUE
alt.allele.other.use.threshold<-0
#make.nonDefined.samples.as.OTHER<-FALSE
include.homozygotes<-TRUE ## used for cancer
###############################################
...
ok.in.group<-as.numeric(summary.geno.group[,"ALT.Alleles.ALL"]) <= (num.affected)*2 # in case is fully recessive
on.xychromo<-indels[,"chr"]=="chrX" | indels[,"chr"]=="chrY"

sum(ok.in.group)
sum(on.xychromo)
extra.counts.for.sex<-as.numeric(summary.geno.group[,"ALT.Alleles.ALL"]) <= (num.affected+3)*2
