...
#the.sample.sheet<-"20131115_GroupCallSKDP_MODY_PCC_ChMND_SampleSheet.csv"
###max.reads <- 70000


skip.annovar.run<-FALSE # if FALSE it just redoes the files annovar summarization IF not already run
update.annovar.annotations<-TRUE # set TRUE will re-read VCF file and force annovar to always run or  FALSE will use the precalcuated Annovar annotation
...
update.annotations.with.vep<-TRUE ##undate gene names and geneanno.table with filter.table.poly... wanted muts traken care of below
skip.gene.list.matches<-FALSE ### usually false . If TRUE it skips  lists of gene names
...
if(!exists("max.reads")){max.reads<-25000} # 0- get ALL IN ONE GO  : 50000 is a good guess
...
db.details<-list(user="gerp2",pass="GeRpUQ8!6", dbname="gerp", host="ga-apps.di.uq.edu.au") #"di-mysql01.di.uq.edu.au"
...
anno.DB.location.core<-"/dmf/uqdi/Core_Services/UQCCG/Software/annovar/humandb"
...
anno.DB.location.core<-"/dmf/uqdi/Core_Services/UQCCG/Software/annovar/mousedb"
anno.DB.location<-paste(anno.DB.location.core,genome.build,sep="/")
...
print(get.cols)
system(
   paste(annovar.excutable.path,"/","annotate_variation.pl -regionanno -colsWanted ",get.cols,"  -buildver ",genome.build," -dbtype ",regionanno.DB[i]," ",target,"  ",anno.DB.location," --outfile ",paste(target,regionanno.DB[i],sep=".")," ",sep="" )
      )
}
...
print(get.cols)
system(
   paste(annovar.excutable.path,"/","annotate_variation.pl -regionanno -colsWanted ",get.cols,"  -buildver ",genome.build," -dbtype ",regionanno.DB.score[i]," ",target,"  ",anno.DB.location," -scorecolumn ", regionanno.DB.score.col[i], " --outfile ",paste(target,regionanno.DB.score[i],sep=".")," ",sep="" )
      )
}
}
...
system(
    paste(annovar.excutable.path,"/","annotate_variation.pl -batchsize 50m -filter --score_threshold ",maf.threshold," -buildver ",genome.build," -dbtype ",filter.DB[i]," ",target,"  ",anno.DB.location," --outfile ",paste(target,filter.DB[i],sep=".")," ",sep="" )
      )
}
############################################################
...
system(
    paste(annovar.excutable.path,"/","annotate_variation.pl -batchsize 1000m -filter --score_threshold ",maf.threshold," -buildver ",genome.build," -dbtype  generic -genericdbfile ",generic.filter.DB[i]," ",target,"  ",anno.DB.location," --outfile ",paste(target,generic.filter.DB[i],sep=".")," ",sep="" )
      )
}
...
system(
    paste(annovar.excutable.path,"/","annotate_variation.pl -batchsize 50m -geneanno --splicing_threshold ",splice.threshold," -buildver ",genome.build," -dbtype ",geneanno.DB[i]," ",target,"  ",anno.DB.location," --outfile ",paste(target,geneanno.DB[i],sep=".")," ",sep="" )
      )
}
...
if(grepl("^chrMT",target) | grepl(".chrMT.",target)){  ## mitochrondrial using 1000 genomes FASTA - which we use replaces the ensGene one written above for mitochromdria (only works if doing per chromosome
 print("running mitochrondria")
  if( (paste(target,geneanno.DB[i],"log",sep=".") %in% files.in.annotate ) & !update.annovar.annotations){next} # perhaps have added batabases but dones want to update
system(
    paste(annovar.excutable.path,"/","annotate_variation.pl -batchsize 50m -geneanno --splicing_threshold ",splice.threshold," -buildver ",genome.build, "-dbtype MT_GRCh37_ensGene ",target,"  ",anno.DB.location," --outfile ",paste(target,"ensGene",sep=".")," ",sep="" )
      )
}
...
target.table[1,]
  ...
  if(force.GERP.update | !(gerp.data.file %in% files.in.annotate) ){
    if(exists("db.details")){db.details<-list(user="gerp2",pass="GeRpUQ8!6", dbname="gerp", host="ga-apps.di.uq.edu.au")} #host="ga-apps.di.uq.edu.au" # di-mysql01.di.uq.edu.au
...
a.bone.file<-match.cols.and.collapse(gene.names,"refGene::gene",a.bone.file,colnames(a.bone.file)[1],colnames(a.bone.file),"delimit") # macth column must in be collapse list
a.bone.file<-apply(a.bone.file,1,function(x) if(x[1]=="NA" | is.na(x[1])){NA}else{paste(x,collapse="::")})
ALS.ji<-as.data.frame(a.bone.file)
colnames(ALS.ji)<-c("ALS.ji")
#}
...
colnames(pot.genes)<-c("Potassium")
  ...
  ## Removed "ALS" as was broken above
   save(list=c("omim","ingenuity.bone.genes","Dequeant.cycling","mouse.defect","sewell.cycling","skeletome","lung.genes","pot.genes","HypoMag","ProtonPump","ALS.ji","ALS","Neurodegen"),file=paste(gsub(".txt$","",target),".GeneLists.RData",sep=""))
...
colnames(AML_OTHER_Mutations)<-colnames(a.bone.file)[1] # unique(AML_ALL_Mutations)
   ...
   save(list=c("omim","ingenuity.bone.genes","Dequeant.cycling","mouse.defect","sewell.cycling","skeletome","lung.genes","pot.genes","HypoMag","ProtonPump","ALS.ji","ALS","Neurodegen","AML_OTHER_Mutations","AML_MITO_Mutations","AML_LEY_RELAPSE_Mutations","AML_FRANC_Mutations","AML_ENU_Mutations","AML_DINDEL_Relapse","AML_SNP_Mutations","AML_DINDEL_Mutations","AML_CLINICAL_Mutations","AML_ASH_Mutations","AML_ALL_Mutations"),file=paste(gsub(".txt$","",target),".GeneLists.RData",sep=""))
} ## project AML
