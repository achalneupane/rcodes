...
########### This ine is for reading genotyoe data generated bu the murcery data format see for example
...
column.labels<-read.delim(sample.files[isamp],header=F,nrows=1,sep="\t",fill=TRUE,stringsAsFactors=FALSE,na.strings="",quote="\"")
...
reads<-max.reads  #1.3M lines in snp file 50000 goes out to 24Gb without QC cgeck  reads<-100
counter<- -1
indels.all<-{}
while (num.lines >0  ){
counter<-counter+1
...
format.types<-unlist(strsplit(unique(indels[,"INFO"]),split=":"))
info.types<-c()
####################################### FINISHED Read in data
indels<- process.GATK.indels.Mercury(indels,sample.files[isamp],vcf.type,format.types,info.types,info.class,num.cores)
