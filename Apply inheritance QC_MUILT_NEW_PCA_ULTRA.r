...
if(!exists("max.reads")){max.reads<-700000 } # 0- get ALL IN ONE GO  :
## NA get all
## 1000 samples - 10000 
...
## num.bits
## all.genotypes<-foreach(indels.bit=iter(indels,by='row',chunksize=as.integer(dim(indels)[1]/num.bits) ), .combine='rbind', .multicombine=TRUE, .inorder=TRUE) %dopar% filtered.genotype(indels.bit,gsub(".GT$","",the.samples),prefix="",suffix=".ALL",20,0.2,0.80,0.05,0.95,10,5,GQ.threshold)

## targets<-c("ALL")s #c("NMD","ex.Control","AOGC")
## targets
...
position.filter.QC<-  position.filter.QC & maf.fil &  missing.ok  & !is.unwound.geno & !are.in.repeats & !are.repeats & hw.p.ok # & inbreeding.ok
...
all.genotypes$"summary.geno.group"<-all.genotypes$"summary.geno.group"[position.filter.QC,]
all.genotypes$"summary.depths.group"<-all.genotypes$"summary.depths.group"[position.filter.QC,]
all.genotypes$"genotypes"<-all.genotypes$"genotypes"[position.filter.QC,]
...
write.plink(indels,paste(project.name,"plink",counter,sep="_"))
...
while((dim(indels)[1] %% num.bits)< 2){num.bits<-num.bits+1} ### go don't get matrix issues
num.bits
if(dim(all.genotypes$"genotypes")[1]<200){num.bits<-1; print(paste("Only",dim(all.genotypes$"genotypes")[1],"genotypes remaining",sep=" ")) }
  
  
print("do Aij calculation")
...
a.single.test<-FALSE
print("first assign")
plink.files.all<-paste(project.name,"plink",counter,sep="_")
}else{
Aij.total<-Aij.total+Aij
plink.files.all<-c(plink.files.all,paste(project.name,"plink",counter,sep="_"))
print("Add Aij")
}
