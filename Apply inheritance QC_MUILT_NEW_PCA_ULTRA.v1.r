...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
project.dir<-"/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/2015-02-11_LeoPharmFinalAnalysisRedoChr1" # path to project data 
#project<-"2015-01-15_LeoPharma_Dec2014Freeze" # this is the project directory /media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/SNPs/2015-02-11_LeoPharmFinalAnalysis.chrX.output.recalibrated.filtered.vcf
project.name<-"2015-02-11_LeoPharmFinalAnalysisRedoChr1" # this is the prefix of the SNP and DINDEL files AND the project output NAME
#/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/2015-02-11_LeoPharmFinalAnalysisRedoChr1/SNPs/2015-02-11_LeoPharmFinalAnalysisRedoChr1.chr1.output.recalibrated.filtered.vcf
...
###### THIS IS the super annotion run USE ALL THE FILTER AND NOVEL DATABASES
project.dir<-"/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-03-16_AllAMLandLung" # FULLpath to project data 
#project<-"2015-01-15_LeoPharma_Dec2014Freeze" # this is the project directory /media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/SNPs/2015-02-11_LeoPharmFinalAnalysis.chrX.output.recalibrated.filtered.vcf
project.name<-"2015-03-16_AllAMLandLung" # this is the prefix of the SNP and DINDEL files AND the project output NAME
#/media/UQCCG/Sequencing/Data/Sequence_Genotypes/2015/2015-02-11_LeoPharmFinalAnalysis/2015-02-11_LeoPharmFinalAnalysisRedoChr1/SNPs/2015-02-11_LeoPharmFinalAnalysisRedoChr1.chr1.output.recalibrated.filtered.vcf


##affection.status<-"sample_sheet" This parameter is not used, not sure why here. Mhairi Feb 2015
the.sample.sheet<-"no_file"
...
if(!exists("max.reads")){max.reads<-25000 } # 0- get ALL IN ONE GO  :
## NA get all
...
final.QC.directory<-"/mnt/UQCCG/Sequencing/Data/QC for all samples summary/Genetic_QC"
...
indels<-  process.GATK.indels(indels,sample.files[isamp],vcf.type,format.types,info.types,info.class,num.cores.ori) ## names(the.sample.file) must be the mutation type
...
## sum(p>0.2)
## check<-(p.20>0.05 ) & !(p>0.05 ) & missing.ok
## sum(check)

## summary.geno.group[check,]
## summary.20[check,]
## p.20<-p
## summary.20<-summary.geno.group
## indels[1:5,paste0(the.samples,".GT")]
...
## all.genotypes$"summary.geno.group"[position.filter.QC & !inbreeding.ok,][1:10,]
## inbreeding.rate[position.filter.QC & !inbreeding.ok][1:10]
## hw.p.ok[position.filter.QC & !inbreeding.ok][1:10 ]
...
#sum(position.filter.QC & inbreeding.ok)
...
## > [1] 30302
## > [1] 29037
## > [1] "FILTER_PASS" "QUAL"        "QD"          "FS"          "HRun"        "SB"          "flat"       
## > [1] 356
## > 
...
############################ACHAL
## getwd()
## print("Write plink binary format")
## write.plink(indels[position.filter.QC,] ##ped/map  paste(gsub(".vcf$","",sample.files[isamp]),"plink",counter,sep="_"))
### equationd from
####################################
## http://www.nature.com/nrg/journal/v11/n11/extref/nrg2865-s1.pdf # use Aij RAW UAR
if(sum(position.filter.QC)>1){
num.bits<-num.cores.ori
while((dim(all.genotypes)[1] %% num.bits)< 2){num.bits<-num.bits+1} ### go don't get matrix issues
num.bits
if(dim(all.genotypes)[1]<200){num.bits<-1; print(paste("Only",dim(all.genotypes)[1],"genotypes remaining",sep=" ")) }
...
currect.dir<-getwd()
############# write OUTPUT
plink.files.all<-cbind(paste0(plink.files.all,".bed"),paste0(plink.files.all,".bim"),paste0(plink.files.all,".fam"))
write.table(plink.files.all,file=paste(project.name.alternative,"plink_file_merge.txt",sep="."),col.names=FALSE,row.names=FALSE,sep="\t",quote=FALSE)
...
#write.table(related.sheet,file=paste(project.name.alternative,"Genetic_QC.txt",sep="."),col.names=TRUE,row.names=FALSE,sep="\t",quote=FALSE)

   
xx<-try(setwd(final.QC.directory))
if(inherits(xx, "try-error")){ ## can't charge directory
setwd(analysis.dir)
}
out.file.name<-paste("Vcf_Merge.","GQ-",GQ.threshold,".Min.ALT-",allele.min,"_",project.name.alternative,".",format(Sys.time(), "%a_%b_%d_%Y"),".genetic_QC.txt",sep="")
...
out.file.name<-paste("Vcf_Merge.","GQ-",GQ.threshold,".Min.ALT-",allele.min,"_",project.name.alternative,".ARE_RELATED.",format(Sys.time(), "%a_%b_%d_%Y"),".genetic_QC.txt",sep="")
...
###########################################################################################################################
###########################################################################################################################
## pop<-read.plink.raw("/media/UQCCG/GWAS/Popularion stratification/ALL_eth_650Y_forward_hg19_Best")
## dim(pop)
## pop[1:5,1:15]
## pop[,1]<-gsub("^ ","",pop[,1])
## table(pop[,1])

## fil.genotypes<- filtered.genotype(pop[1:8,],the.samples.fam,prefix="",suffix="",20,0.2,0.80,0.05,0.95,10,5)
